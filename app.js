...
const modalFocusBack=new WeakMap();
function visibleModals(){return [...document.querySelectorAll('.modal:not(.hidden)')]}
function modalFocusable(modal){
  return [...modal.querySelectorAll('input:not([type="hidden"]):not([disabled]),select:not([disabled]),textarea:not([disabled]),button:not([disabled]),a[href],[tabindex]:not([tabindex="-1"])')]
    .filter(el=>!el.closest('.hidden')&&el.getAttribute('aria-hidden')!=='true');
}
function refreshModalLayer(){
  const list=visibleModals(),top=list[list.length-1];
  document.body.classList.toggle('modal-open',list.length>0);
  list.forEach(modal=>{
    const active=modal===top;
    modal.setAttribute('aria-hidden',active?'false':'true');
    if(active)modal.setAttribute('aria-modal','true');else modal.removeAttribute('aria-modal');
  });
}
function labelModal(modal){
  ...
  const labelled=modal.querySelector('[aria-label]');
  if(!modal.hasAttribute('aria-label'))modal.setAttribute('aria-label',labelled?.getAttribute('aria-label')||'Cửa sổ');
}
function openModal(id){
  const modal=document.getElementById(id);if(!modal)return;
  modalFocusBack.set(modal,document.activeElement);
  modal.dataset.dirty='0';
  modal.setAttribute('role','dialog');modal.tabIndex=-1;labelModal(modal);
  modal.classList.remove('hidden');
  refreshModalLayer();
  requestAnimationFrame(()=>{
    const preferred=modal.querySelector('[autofocus],form input:not([type="hidden"]):not([disabled]),form select:not([disabled]),form textarea:not([disabled])');
    (preferred||modalFocusable(modal)[0]||modal).focus?.();
  });
}
function closeModal(id){
  const modal=document.getElementById(id);if(!modal)return;
  modal.classList.add('hidden');modal.setAttribute('aria-hidden','true');
  modal.removeAttribute('aria-modal');delete modal.dataset.dirty;
  modal.querySelectorAll('form').forEach(form=>form.reset());
  refreshModalLayer();
  const back=modalFocusBack.get(modal),top=visibleModals().pop();
  if(back&&document.contains(back)&&(!top||top.contains(back)))back.focus({preventScroll:true});
  else if(top)(modalFocusable(top)[0]||top).focus?.({preventScroll:true});
}
function requestCloseModal(id){
  const modal=document.getElementById(id);if(!modal||modal.classList.contains('hidden'))return;
  if(modal.querySelector('.is-busy')){showToast('Thao tác đang được xử lý, vui lòng chờ hoàn tất.');return}
  if(modal.dataset.dirty==='1'&&!confirm('Cửa sổ này có dữ liệu chưa lưu. Đóng và bỏ các thay đổi?'))return;
  closeModal(id);
}
...
const demoData={
  ...
  rooms:[
    ...
    {id:'r2',propertyId:'p1',name:'P102',price:2500000,deposit:2500000,area:25,status:'available',type:'Phòng trọ',capacity:2,amenities:['Điều hòa','Nóng lạnh','Ban công'],note:'',electricRate:3500,waterMode:'meter',waterRate:15000,waterFixed:0,imageIds:[]},
...
function migrateData(x){
  ...
  d.leases.forEach(l=>{l.status=l.status||'draft';l.rentAmount=Number(l.rentAmount||0);
    ...
    l.documentFiles=Array.isArray(l.documentFiles)?l.documentFiles:[]});
  ...
  ['properties','rooms','tenants','utilityReadings','invoices','appointments','reservations','leases','leaseOccupants','accounts','assets','handoverItems','serviceDefinitions','leaseServices','payments','depositLedger','reminders','maintenanceTickets','notifications','staffUsers'].forEach(k=>{
  ...
  d.depositLedger.forEach(x=>{x.amount=Number(x.amount||0);x.reservationId=x.reservationId||'';x.roomId=x.roomId||'';x.tenantId=x.tenantId||'';x.appointmentId=x.appointmentId||'';x.reference=x.reference||''});
  ...
  d.tenants.forEach(t=>{t.holdRoomId=t.holdRoomId||'';t.holdFrom=t.holdFrom||'';t.holdUntil=t.holdUntil||'';t.holdAmount=Number(t.holdAmount||0);t.holdNote=t.holdNote||''});
  ...
  d.appointments.forEach(a=>{
    ...
    // P2 chuẩn hóa phễu còn đúng 7 bước. Trạng thái "soạn hợp đồng" cũ
    // được thể hiện bằng convertedLeaseId, không còn là một cột riêng trong phễu.
    if(a.status==='lease_draft')a.status='viewed';
  ...
  d.reservations.forEach(r=>{
    r.sourceType=r.sourceType||'';r.sourceId=r.sourceId||'';r.appointmentId=r.appointmentId||'';r.tenantId=r.tenantId||'';
    r.customerName=r.customerName||'';r.customerPhone=r.customerPhone||'';r.fromDate=r.fromDate||today();r.untilDate=r.untilDate||'';
    r.amount=Number(r.amount||0);r.paymentMethod=r.paymentMethod||'cash';r.paymentReference=r.paymentReference||'';
    r.note=r.note||'';r.status=r.status||'active';r.depositEntryId=r.depositEntryId||'';r.leaseId=r.leaseId||'';
    r.cancelledAt=r.cancelledAt||'';r.cancelReason=r.cancelReason||'';r.createdBy=r.createdBy||'migration';r.createdAt=r.createdAt||new Date().toISOString();
  });
  ...
  migrateReservationsLocal(d);
  migrateLeasesLocal(d);
  migrateBillingLocal(d);
  repairLifecycleLocal(d);
  d.rooms.forEach(r=>reconcileRoomInData(d,r.id));
  return d;
}
...
/** v4.4 P0: gom hai kiểu giữ chỗ cũ (CRM / người thuê) vào một collection reservations.
 * ID và bút toán dùng mã cố định nên chạy lại nhiều lần không tạo trùng. */
function migrateReservationsLocal(d){
  const ids=new Set(d.reservations.map(r=>r.id));
  const depIds=new Set(d.depositLedger.map(x=>x.id));
  const addLegacy=(res,shouldCreateLedger)=>{
    if(!ids.has(res.id)){d.reservations.push(res);ids.add(res.id)}
    if(!shouldCreateLedger)return;
    const depId=res.depositEntryId||('dep_'+res.id);
    if(!res.depositEntryId)res.depositEntryId=depId;
    if(depIds.has(depId)||d.depositLedger.some(x=>x.reservationId===res.id&&x.type==='collect'))return;
    d.depositLedger.push({id:depId,leaseId:res.leaseId||'',reservationId:res.id,roomId:res.roomId,
      tenantId:res.tenantId||'',appointmentId:res.appointmentId||'',type:'collect',amount:Number(res.amount||0),
      at:String(res.createdAt||today()).slice(0,10)||today(),method:res.paymentMethod||'cash',reference:res.paymentReference||'',
      note:'Chuyển từ dữ liệu giữ chỗ cũ'+(res.note?' — '+res.note:''),createdBy:'migration',createdAt:res.createdAt||new Date().toISOString()});
    depIds.add(depId);
  };
  d.appointments.forEach(a=>{
    if(!(Number(a.reserveAmount)>0)||!a.reserveUntil)return;
    const existing=d.reservations.find(r=>r.appointmentId===a.id);
    if(existing){addLegacy(existing,true);return}
    const leaseId=a.convertedLeaseId||'';
    const status=leaseId?'converted':'active';
    addLegacy({id:'res_mig_appt_'+a.id,roomId:a.roomId,sourceType:'appointment',sourceId:a.id,appointmentId:a.id,tenantId:'',
      customerName:a.customerName||'',customerPhone:a.customerPhone||'',fromDate:String(a.createdAt||a.date||today()).slice(0,10),
      untilDate:a.reserveUntil,amount:Number(a.reserveAmount),paymentMethod:'cash',paymentReference:'',note:'Chuyển từ CRM cũ',
      status,depositEntryId:'dep_res_mig_appt_'+a.id,leaseId,cancelledAt:'',cancelReason:'',createdBy:'migration',createdAt:a.createdAt||new Date().toISOString()},true);
  });
  d.tenants.forEach(t=>{
    if(!t.holdRoomId||!(Number(t.holdAmount)>0)||!t.holdUntil||activeLeaseForTenantIn(d,t.id))return;
    const existing=d.reservations.find(r=>r.tenantId===t.id);
    if(existing){addLegacy(existing,true);if(existing.status==='active')t.active=false;return}
    addLegacy({id:'res_mig_ten_'+t.id,roomId:t.holdRoomId,sourceType:'tenant',sourceId:t.id,appointmentId:'',tenantId:t.id,
      customerName:t.name||'',customerPhone:t.phone||'',fromDate:t.holdFrom||today(),untilDate:t.holdUntil,
      amount:Number(t.holdAmount),paymentMethod:'cash',paymentReference:'',note:t.holdNote||'Chuyển từ người thuê cũ',status:'active',
      depositEntryId:'dep_res_mig_ten_'+t.id,leaseId:'',cancelledAt:'',cancelReason:'',createdBy:'migration',createdAt:new Date().toISOString()},true);
    t.active=false; // v4.3 từng bật active khi giữ chỗ; không để migration hiểu nhầm là đã nhận phòng
  });
  // Tự phục hồi phiếu P0 cũ/cộng dữ liệu thiếu bút toán thu.
  d.reservations.forEach(r=>{if(Number(r.amount)>0)addLegacy(r,true)});
  // Phiếu quá hạn vẫn khóa phòng cho tới khi quản lý xác nhận hoàn/giữ tiền.
  d.rooms.forEach(room=>reconcileRoomInData(d,room.id));
}
function activeLeaseForTenantIn(d,tenantId){return (d.leases||[]).find(l=>(l.status==='active'||l.status==='ending')&&l.primaryTenantId===tenantId)}
function leaseHeldInData(d,leaseId){
  const linked=new Set((d.reservations||[]).filter(r=>r.leaseId===leaseId).map(r=>r.id));
  return (d.depositLedger||[]).filter(x=>x.leaseId===leaseId||(x.reservationId&&linked.has(x.reservationId)))
    .reduce((sum,x)=>sum+(x.type==='collect'?1:-1)*Number(x.amount||0),0);
}
function reconcileRoomInData(d,roomId){
  const room=(d.rooms||[]).find(r=>r.id===roomId);if(!room)return;
  const occupied=(d.leases||[]).some(l=>l.roomId===roomId&&['active','ending'].includes(l.status))||
    (d.tenants||[]).some(t=>t.roomId===roomId&&t.active);
  const held=(d.reservations||[]).some(r=>r.roomId===roomId&&r.status==='active')||
    (d.leases||[]).some(l=>l.roomId===roomId&&l.status==='draft'&&leaseHeldInData(d,l.id)>0);
  room.status=occupied?'occupied':held?'reserved':room.status==='maintenance'?'maintenance':'available';
}
/** Sửa idempotent dữ liệu từng bị lưu trữ người đại diện khi hợp đồng vẫn còn sống. */
function repairLifecycleLocal(d){
  (d.leases||[]).filter(l=>['active','ending'].includes(l.status)).forEach(l=>{
    const tenant=(d.tenants||[]).find(t=>t.id===l.primaryTenantId);if(!tenant)return;
    const links=(d.leaseOccupants||[]).filter(x=>x.leaseId===l.id);
    let primary=links.find(x=>x.occupantId===l.primaryTenantId);
    if(!primary){
      primary={id:uid('lofix_'),leaseId:l.id,occupantId:l.primaryTenantId,role:'primary',joinedAt:l.moveInAt||l.startDate||'',leftAt:'',note:'Tự sửa liên kết đại diện',createdAt:new Date().toISOString()};
      d.leaseOccupants.push(primary);
    }
    links.forEach(x=>{if(x.id!==primary.id&&x.role==='primary'&&!x.leftAt)x.role='member'});
    primary.role='primary';primary.leftAt='';
    tenant.active=true;tenant.roomId=l.roomId;tenant.moveOutDate='';
  });
}
...
function depositEntries(leaseId){
  const linkedReservations=new Set(data.reservations.filter(r=>r.leaseId===leaseId).map(r=>r.id));
  return data.depositLedger.filter(x=>x.leaseId===leaseId||(x.reservationId&&linkedReservations.has(x.reservationId)))
    .sort((a,b)=>String(a.at).localeCompare(b.at));
}
...
function leaseDepositHeld(leaseId){return Math.max(0,depositTotals(leaseId).held)}
function leaseDepositCollected(leaseId){return Math.max(0,depositTotals(leaseId).collect)}
function pushLeaseStatus(l,type,at,detail){
  l.statusHistory=Array.isArray(l.statusHistory)?l.statusHistory:[];
  l.statusHistory.push({type,at:at||today(),by:currentStaffName(),detail:String(detail||'')});
}
function addDepositEntry(leaseId,type,amount,method,note,at){
  const e={id:uid('dep'),leaseId,reservationId:'',roomId:getLease(leaseId)?.roomId||'',tenantId:getLease(leaseId)?.primaryTenantId||'',appointmentId:'',
    type,amount:Number(amount||0),at:at||today(),method:method||'cash',reference:'',note:String(note||''),createdBy:'admin',createdAt:new Date().toISOString()};
  data.depositLedger.push(e);
  const l=getLease(leaseId);
  // Trường depositPaid chỉ giữ số đã nhận để tương thích dữ liệu cũ; số dư lấy từ sổ cọc.
  if(l){const t=depositTotals(leaseId);l.depositPaid=t.collect}
  return e;
}
  ...
  d.tenants.forEach(t=>{
    ...
    if(!lease){
      ...
      lease={id:'l_mig_'+t.id,propertyId:room.propertyId||'',roomId:t.roomId,primaryTenantId:t.id,
        ...
        depositDeduct:0,depositRefund:0,settlementNote:'',roomHistory:[],renewals:[],statusHistory:[],documentFiles:[]};
      d.leases.push(lease);
    }
...
function getReservation(id){return data.reservations.find(x=>x.id===id)}
...
function roomHasHistory(roomId){return data.tenants.some(t=>t.roomId===roomId)||data.utilityReadings.some(u=>u.roomId===roomId)||data.invoices.some(i=>i.roomId===roomId)||data.leases.some(l=>l.roomId===roomId)||data.reservations.some(r=>r.roomId===roomId)}
function dateRangesOverlap(aFrom,aUntil,bFrom,bUntil){return String(aFrom||'')<=String(bUntil||'')&&String(bFrom||'')<=String(aUntil||'')}
function reservationOpen(r){return !!(r&&r.status==='active')}
function reservationActive(r){return !!(r&&r.status==='active'&&r.untilDate&&r.untilDate>=today())}
function reservationExpired(r){return !!(r&&r.status==='active'&&r.untilDate&&r.untilDate<today())}
function reservationForAppointment(id){return data.reservations.find(r=>r.appointmentId===id&&['active','converted'].includes(r.status))||data.reservations.find(r=>r.appointmentId===id)}
function reservationForTenant(id){return data.reservations.find(r=>r.tenantId===id&&r.status==='active')||data.reservations.find(r=>r.tenantId===id&&r.status==='converted')}
function activeReservationForRoom(roomId,from='',until='',exceptId=''){
  return data.reservations.find(r=>r.id!==exceptId&&r.roomId===roomId&&reservationOpen(r)&&(!from||!until||dateRangesOverlap(r.fromDate,r.untilDate,from,until)));
}
function reservationLedgerEntries(reservationId){return data.depositLedger.filter(x=>x.reservationId===reservationId)}
function reservationBalance(reservationId){
  return reservationLedgerEntries(reservationId).reduce((sum,x)=>sum+(x.type==='collect'?1:-1)*Number(x.amount||0),0);
}
/** Hàm trung tâm tính lại trạng thái phòng theo người thuê đang hoạt động. */
function reconcileRoomStatus(roomId){
  reconcileRoomInData(data,roomId);
}
...
function publicRooms(){
  // Chỉ bán phòng trống, sắp trống hoặc đang giữ chỗ. Phòng đang thuê lâu dài
  // không xuất hiện trên trang khách để tránh nhận lead không thể phục vụ.
  return visibleRooms().filter(r=>r.status!=='maintenance'&&(r.status!=='occupied'||roomSoonAvailable(r)));
}
...
async function renderRoomPage(slug){
  ...
  grid.innerHTML=`<div class="detail-page" style="grid-column:1/-1">
    ...
    ${mapCta(p?.address)}
    ${ctaRow(r.id,r.status==='available'||roomSoonAvailable(r))}
  </div>`;
  await hydrateImages(grid);
}
...
function mapCta(address){
  const href=P2.mapUrl(address);if(!href)return'';
  return `<div class="detail-map"><div>${icon('pin',17)}<span><strong>Vị trí căn trọ</strong><small>${esc(address)}</small></span></div><a class="btn btn-light" href="${href}" target="_blank" rel="noopener">${icon('pin',15)} Mở Google Maps</a></div>`;
}
let galleryState={roomId:'',ids:[],index:0};
async function renderGallery(){
  const r=getRoom(galleryState.roomId);if(!r)return;
  const p=getProperty(r.propertyId),ids=galleryState.ids,index=Math.max(0,Math.min(galleryState.index,Math.max(0,ids.length-1)));
  galleryState.index=index;
  document.getElementById('galleryContent').innerHTML=`<div class="gallery-head"><div><h3>${esc(p?.name||'')} · ${esc(r.name)}</h3><p>${money(r.price)}/tháng · Cọc ${money(r.deposit)} · ${r.area||'-'} m²</p></div>${ids.length?`<strong>${index+1}/${ids.length}</strong>`:''}</div>
  ${ids.length?`<div class="gallery-stage"><button class="gallery-nav gallery-prev" data-evt="click" data-call="galleryStep" data-a1="-1" aria-label="Ảnh trước">${icon('back',24)}</button><img class="image-loading" data-image-id="${esc(ids[index])}" alt="Ảnh ${index+1} của phòng ${esc(r.name)}"><button class="gallery-nav gallery-next" data-evt="click" data-call="galleryStep" data-a1="1" aria-label="Ảnh sau">${icon('back',24,'flip')}</button></div><div class="gallery-thumbs" role="list" aria-label="Chọn ảnh">${ids.map((id,i)=>`<button class="${i===index?'active':''}" data-evt="click" data-call="gallerySelect" data-a1="${i}" aria-label="Xem ảnh ${i+1}" aria-current="${i===index?'true':'false'}"><img class="image-loading" data-image-id="${esc(id)}" alt=""></button>`).join('')}</div>`:'<div class="gallery-empty">Phòng này chưa có ảnh. Quản lý có thể tải ảnh lên trong phần Căn trọ & phòng.</div>'}
  <div class="gallery-info">${(r.amenities||[]).map(a=>`<span>${esc(a)}</span>`).join('')}</div>`;
  await hydrateImages(document.getElementById('galleryContent'));
}
window.openGallery=async function(roomId){const r=getRoom(roomId);if(!r)return;const p=getProperty(r.propertyId);const ids=[...(r.imageIds||[])];if(!ids.length&&p?.imageIds?.length)ids.push(...p.imageIds);galleryState={roomId,ids,index:0};openModal('galleryModal');await renderGallery()}
window.galleryStep=function(delta){if(!galleryState.ids.length)return;galleryState.index=(galleryState.index+Number(delta)+galleryState.ids.length)%galleryState.ids.length;renderGallery()}
window.gallerySelect=function(index){galleryState.index=Number(index)||0;renderGallery()}
function workSlots(){
...
let bookingSlotRequest=0;
function paintBookingSlots(busy){
  const sel=document.getElementById('appointmentTime');if(!sel)return;
  const prev=sel.value,slots=workSlots();
  sel.innerHTML=slots.map(t=>`<option value="${t}" ${busy.has(t)?'disabled':''}>${t}${busy.has(t)?' — đã có khách hẹn':''}</option>`).join('');
  const firstFree=slots.find(t=>!busy.has(t));
  sel.value=(prev&&!busy.has(prev))?prev:(firstFree||slots[0]||'09:00');
}
window.refreshBookingSlots=async function(){
  const roomId=document.getElementById('bookingRoomId').value;
  const date=document.getElementById('appointmentDate').value;
  let busy=new Set(data.appointments.filter(a=>a.roomId===roomId&&a.date===date&&!['cancelled','lost'].includes(a.status)).map(a=>a.time));
  paintBookingSlots(busy);
  if(!roomId||!date||!Sync.isOn())return busy;
  const requestId=++bookingSlotRequest;
  try{
    const out=await Sync.publicAvailability(roomId,date);
    if(requestId!==bookingSlotRequest||document.getElementById('bookingRoomId').value!==roomId||document.getElementById('appointmentDate').value!==date)return busy;
    busy=new Set(Array.isArray(out.busyTimes)?out.busyTimes:[]);
    paintBookingSlots(busy);
  }catch(err){/* máy chủ vẫn kiểm tra nguyên tử khi gửi; giữ danh sách cục bộ nếu mất mạng */}
  return busy;
}
...
function localAdminGranted(){return !Sync.isOn()&&localStorage.getItem('huyrooms_admin_v3')==='true'}
function showAdmin(){
  // v4.1: không cho vào màn quản trị khi chưa xác thực (online = token, offline = cờ máy này)
  if(!(Sync.isAdmin&&Sync.isAdmin())&&!localAdminGranted()){openModal('loginModal');return}
...
async function showResident(tenantId){
  if(tenantId)currentResidentId=tenantId;
  document.getElementById('publicApp').classList.add('hidden');document.getElementById('publicTopbar').classList.add('hidden');document.getElementById('adminApp').classList.add('hidden');document.getElementById('residentApp').classList.remove('hidden');window.scrollTo(0,0);
  if(!residentSession){renderResident();return}
  if(!Sync.isOn()){
    residentValidated=false;saveResidentSession(null);
    document.getElementById('residentContent').innerHTML='<div class="rs-error"><h3>Cần kết nối để xác minh tài khoản</h3><p>Dữ liệu cư dân đã lưu không được hiển thị khi chưa xác minh với máy chủ.</p><button class="btn btn-primary" onclick="showPublic();openModal(\'residentLoginModal\')">Đăng nhập lại</button></div>';
    return;
  }
  if(residentValidated){renderResident();return}
  document.getElementById('residentContent').innerHTML='<div class="rs-error"><h3>Đang xác minh phiên đăng nhập…</h3><p>Vui lòng chờ trong giây lát.</p></div>';
  if(await validateResidentSession())renderResident();
}
let residentValidated=false;
async function validateResidentSession(){
  if(!residentSession||!residentSession.sessionKey||!Sync.isOn())return false;
  if(residentValidated)return true;
  try{
...
function RD(){
  ...
  return {online:false,t,r,p:getProperty(r?.propertyId),lease,co,
    ...
    dep:lease?depositEntries(lease.id):[],
...
function rsHome(rd,notices){
  ...
  <div class="resident-grid">
    ...
    <div class="resident-card ${debt>0?'rs-due':''}"><h3>Cần thanh toán</h3><div class="big-number">${money(debt)}</div>
      ${nearest?`<div class="kv"><span>Hạn gần nhất</span><strong>${esc(nearest.dueDate)} (${daysUntil(nearest.dueDate)<0?'quá hạn '+Math.abs(daysUntil(nearest.dueDate))+' ngày':daysUntil(nearest.dueDate)===0?'hôm nay':'còn '+daysUntil(nearest.dueDate)+' ngày'})</strong></div>`:`<div class="kv"><span>Không có hóa đơn chờ</span><strong class="success-inline">${icon('check',18)} Đã thanh toán đủ</strong></div>`}
    ...
    <div class="resident-card"><h3>Tài khoản & bảo mật</h3>
      <div class="table-actions" style="flex-direction:column;align-items:stretch;gap:8px">
        <button class="btn btn-light" onclick="openPinChange()">${icon('key',16)} Đổi mã PIN</button>
        ${rd.online?`<button class="btn btn-light" onclick="residentLogoutAllDevices()">${icon('logout',16)} Đăng xuất mọi thiết bị</button>`:''}
        <button class="btn btn-light" onclick="clearResidentDevice()">${icon('trash',16)} Xóa dữ liệu trên thiết bị này</button>
      </div></div>
  </div>`;
}
/* ---------- Hóa đơn ---------- */
...
window.openResidentInvoice=function(invId){
  ...
  document.getElementById('rsInvoiceBody').innerHTML=`
    ...
    ${u&&(u.imageIds||[]).length?`<div style="margin:10px 0"><button class="icon-btn" data-evt="click" data-call="openResidentMeterPhotos" data-a1="${u.id}">${icon('camera',15)} Xem ${u.imageIds.length} ảnh công tơ tháng này</button></div>`:''}
    <h4 class="lease-section-title">Lịch sử thanh toán</h4>
    ${txs.length?txs.map(pp=>`<div class="pay-tx ${pp.kind==='reversal'?'pay-rev':''} ${pp.reversedAt?'pay-reversed':''}"><div><strong>${icon(pp.kind==='reversal'?'back':'receipt',14)} ${pp.kind==='reversal'?'Điều chỉnh':'Đã thu'} ${money(Math.abs(pp.amount))}</strong> · ${esc(pp.paidAt)}${pp.reversedAt?' <span class="badge badge-unpaid">Đã hủy</span>':''}</div></div>`).join(''):'<div class="rs-empty">Chưa có lần thanh toán nào.</div>'}
    ...
    <div class="table-actions" style="margin-top:12px"><button class="btn btn-primary" data-evt="click" data-call="openInvoicePdf" data-a1="${i.id}">${icon('download',16)} Tải / In PDF</button><button class="btn btn-light" data-evt="click" data-call="copyInvoiceText" data-a1="${i.id}">Sao chép chi tiết</button></div>`;
...
function rsLease(rd){
  ...
  return `<div class="resident-grid">
    ...
    <div class="resident-card"><h3>Sổ cọc của hợp đồng</h3>
      ${rd.dep.length?rd.dep.map(x=>`<div class="rem-log">${icon(x.type==='collect'?'receipt':x.type==='refund'?'back':'alert',14)} ${({collect:'Đóng cọc',refund:'Được hoàn',deduct:'Bị trừ'})[x.type]} ${money(x.amount)} · ${esc(x.at)}${x.note?` · ${esc(x.note)}`:''}</div>`).join(''):'<div class="rs-empty">Chưa có giao dịch cọc.</div>'}</div>
  </div>`;
}
/* ---------- Sự cố ---------- */
...
document.getElementById('pinChangeForm').addEventListener('submit',async e=>{
  ...
  if(residentSession){
    ...
    try{
      ...
      residentValidated=false;saveResidentSession(null);closeModal('pinChangeModal');
...
window.residentLogoutAllDevices=async function(){
  ...
  try{
    ...
    residentValidated=false;saveResidentSession(null);showPublic();
...
function switchAdminView(view){
  document.getElementById('fabSheet')?.classList.remove('show');
  ...
  document.getElementById('adminTitle').textContent=({dashboard:'Tổng quan',properties:'Căn trọ & phòng',leases:'Hợp đồng thuê',tenants:'Người ở & tài khoản',utilities:'Điện & nước',invoices:'Hóa đơn',appointments:'Khách & lịch hẹn',tickets:'Sự cố & thông báo',settings:'Cài đặt'})[view]||view;
  renderAdmin(view);
}
document.querySelectorAll('.side-link').forEach(b=>b.addEventListener('click',()=>switchAdminView(b.dataset.view)));
function applyRoleActionUI(){
  const desktop={quickAddTenant:can('create','tenants'),quickAddUtility:can('create','utilityReadings'),quickAddInvoice:can('create','invoices')};
  Object.entries(desktop).forEach(([id,allowed])=>document.getElementById(id)?.classList.toggle('hidden',!allowed));
  const mobile={property:can('create','properties'),room:can('create','rooms'),tenant:can('create','tenants'),utility:can('create','utilityReadings'),invoice:can('create','invoices')};
  document.querySelectorAll('#fabSheet [data-fab]').forEach(btn=>{if(btn.dataset.fab!=='close')btn.classList.toggle('hidden',!mobile[btn.dataset.fab])});
  document.getElementById('mobileFab')?.classList.toggle('hidden',!Object.values(mobile).some(Boolean));
}
function currentAdminView(){
  return document.querySelector('.admin-view.active')?.id?.replace('view-','')||'dashboard';
}
function renderAdminView(view){
  const renderers={
    dashboard:renderDashboard,properties:renderPropertyAdmin,leases:renderLeases,
    tenants:renderTenants,utilities:renderUtilities,invoices:renderInvoices,
    appointments:renderAppointments,tickets:renderTicketsAdmin,settings:renderSettings
  };
  (renderers[view]||renderers.dashboard)();
}
function renderAdmin(){
  const view=arguments[0]||currentAdminView();
  renderAdminView(view);
  applyRoleActionUI();
  const root=document.getElementById('view-'+view)||document.getElementById('adminApp');
  applyTableLabels(root);syncTabbar();hydrateImages(root);
}
...
function svgHBars(items){
  ...
  return `<div class="hbars">${items.map(x=>`<div class="hbar-row"><span class="hbar-name">${esc(x.name)}</span><div class="hbar-track"><i style="width:${Math.round(x.value/max*100)}%"></i></div><strong>${money(x.value)}</strong></div>`).join('')||`<div class="rs-empty success-inline">${icon('check',17)} Không có công nợ</div>`}</div>`;
}
...
function renderDashboard(){
  ...
  // P2: công suất chỉ tính phòng thực sự có người thuê; giữ chỗ được báo riêng.
  const occRate=P2.occupancyRate(rooms);
  ...
  const openTickets=data.maintenanceTickets.filter(k=>ticketOpen(k)&&(!propId||getRoom(k.roomId)?.propertyId===propId));
  const overdueTickets=openTickets.filter(k=>(Date.now()-new Date(k.createdAt||0).getTime())>3*86400000);
  const months=last6Months(reportState.month);
  const rev=months.map(m=>revenueStats(m,propId));
  const debtProps=propId?props.filter(p=>p.id===propId):props;
  const debtByProp=debtProps.map(p=>({name:p.name,value:data.invoices
    .filter(i=>i.month===reportState.month&&getRoom(i.roomId)?.propertyId===p.id&&effectiveInvoiceStatus(i)!=='paid')
  ...
  const metric=(label,val,sub,warn,secondary=false)=>`<div class="metric ${warn?'metric-warn':''} ${secondary?'metric-secondary':''}"><small>${label}</small><strong>${val}</strong>${sub?`<div class="trend">${sub}</div>`:''}</div>`;
  const taskItems=[];
  data.invoices.filter(i=>effectiveInvoiceStatus(i)!=='paid'&&daysUntil(i.dueDate)<=3&&(!propId||getRoom(i.roomId)?.propertyId===propId)).slice(0,4).forEach(i=>{const t=getTenant(i.tenantId),r=getRoom(i.roomId);const d=daysUntil(i.dueDate);
    taskItems.push(`<div class="tk-row"><div class="tk-main"><strong>${esc(t?.name||'')} · ${esc(r?.name||'')} · còn ${money(remainingInvoice(i))}</strong><small>${d<0?'Quá hạn '+Math.abs(d)+' ngày':d===0?'Đến hạn hôm nay':'Còn '+d+' ngày'} · HĐ ${esc(i.month)}</small></div><div class="table-actions">${can('create','reminders')?`<button class="icon-btn" data-evt="click" data-call="openReminder" data-a1="${i.id}">${icon('bell',14)} Nhắc</button>`:''}${can('approve','payments')?`<button class="icon-btn" data-evt="click" data-call="recordPayment" data-a1="${i.id}">Thanh toán</button>`:''}</div></div>`)});
  newAppts.slice(0,3).forEach(a=>{const r=getRoom(a.roomId);taskItems.push(`<div class="tk-row"><div class="tk-main"><strong>${esc(a.customerName)} · ${esc(a.customerPhone)}</strong><small>Hẹn xem ${esc(r?.name||'')} · ${esc(a.date)} ${esc(a.time)}</small></div><div class="table-actions"><button class="icon-btn" onclick="switchAdminView('appointments')">Mở CRM</button></div></div>`)});
  expiring.slice(0,3).forEach(l=>{const t=getTenant(l.primaryTenantId),r=getRoom(l.roomId);taskItems.push(`<div class="tk-row"><div class="tk-main"><strong>HĐ ${esc(t?.name||'')} · ${esc(r?.name||'')}</strong><small>Hết hạn ${esc(l.endDate)} — trao đổi gia hạn sớm</small></div><div class="table-actions"><button class="icon-btn" data-evt="click" data-call="openLeaseDetail" data-a1="${l.id}">Chi tiết</button></div></div>`)});
  openTickets.slice(0,3).forEach(k=>{const r=getRoom(k.roomId);taskItems.push(`<div class="tk-row ${k.priority==='urgent'?'tk-urgent':''}"><div class="tk-main"><strong>${icon('wrench',14)} ${esc(k.title)} · ${esc(r?.name||'')}</strong><small>${TICKET_STATUS[k.status]||'Chưa xử lý'} · ${esc(String(k.createdAt||'').slice(0,10))}</small></div><div class="table-actions"><button class="icon-btn" onclick="switchAdminView('tickets')">Mở sự cố</button></div></div>`)});
  ...
  <div class="panel dashboard-summary">
    ...
    <div class="dashboard-grid metric-grid metric-grid-primary" aria-label="Chỉ số cần chú ý">
      ...
      ${metric('TỶ LỆ LẤP ĐẦY',occRate+'%',`${nOcc} đang thuê · ${nRes} đang giữ chỗ`)}
      ${metric('LỊCH HẸN MỚI',newAppts.length,newAppts.length?'Chờ liên hệ':'Không có',newAppts.length>0)}
      ${metric('SỰ CỐ CHƯA XỬ LÝ',openTickets.length,overdueTickets.length?`${overdueTickets.length} việc mở quá 3 ngày`:'Không có việc quá hạn',overdueTickets.length>0)}
    </div>
  </div>
  <div class="panel priority-panel"><div class="panel-head"><div><h3>Việc cần xử lý hôm nay</h3><p>Công nợ đến hạn, khách mới, hợp đồng sắp hết và sự cố đang mở</p></div>${taskItems.length?`<span class="priority-count">${taskItems.length} việc</span>`:''}</div>
    <div class="priority-list">${taskItems.join('')||emptyState('check','Hôm nay gọn gàng','Không có việc gấp. Có thể xem lại CRM khách hoặc chốt điện nước tháng.',`<button class="btn btn-light" onclick="switchAdminView('utilities')">Mở chốt điện nước</button>`)}</div>
  </div>
  <div class="dashboard-secondary-block">
    <div class="dashboard-section-title"><strong>Chỉ số tham khảo</strong><span>Quy mô, dự kiến và cảnh báo sắp tới</span></div>
    <div class="dashboard-grid metric-grid metric-grid-secondary" aria-label="Chỉ số tham khảo">
      ${metric('TỔNG PHÒNG',nRooms,`${nAvail} trống · ${nRes} giữ chỗ · ${nMaint} bảo trì`,false,true)}
      ${metric('DOANH THU DỰ KIẾN',money(stats.phaiThu),'Tháng '+reportState.month+' · không gồm cọc',false,true)}
      ${metric('CỌC ĐANG GIỮ',money(stats.cocDangGiu),'Sổ cọc — ngoài doanh thu',false,true)}
      ${metric('HĐ SẮP HẾT HẠN',expiring.length,'Trong 30 ngày tới',expiring.length>0,true)}
      ${metric('PHÒNG SẮP TRỐNG',soon.length,'Có thể chào khách mới',false,true)}
    </div>
  </div>
...
function renderPropertyAdmin(){
  ...
  const editProperties=can('edit','properties'),editRooms=can('edit','rooms');
  root.innerHTML=`<div class="panel-head"><div><h3>Danh sách căn trọ</h3><p>${editRooms?'Bấm vào phòng để mở bảng sửa — thông tin chỉ lưu khi bấm nút Lưu trong bảng đó':'Bạn đang ở chế độ chỉ xem đối với căn trọ và phòng'}</p></div>${can('create','properties')?`<button class="btn btn-primary" onclick="openPropertyForm()">+ Thêm căn</button>`:''}</div>`
    ...
    return `<div class="property-admin-card${open?' pa-open':''}">
      ...
      <div class="property-admin-head pa-detail">
        ...
        <div class="property-admin-actions">${p.archived
          ?(editProperties?`<button class="icon-btn" data-evt="click" data-call="restoreProperty" data-a1="${p.id}">Khôi phục</button>`:'')
          :`${can('create','rooms')?`<button class="icon-btn" data-evt="click" data-call="openRoomForm" data-a1="null" data-a2="${p.id}">+ Phòng</button>`:''}${editProperties?`<button class="icon-btn" data-evt="click" data-call="openPropertyForm" data-a1="${p.id}">Sửa & ảnh</button><button class="icon-btn danger" data-evt="click" data-call="deleteProperty" data-a1="${p.id}">Lưu trữ / Xóa</button>`:''}`}
        </div>
      </div>
      <div class="property-admin-body pa-detail">
        <div class="room-admin-list">${rooms.length?rooms.map(r=>{
          ...
          return `<div class="ra-item${r.archived?' ra-archived':''}">
            <button type="button" class="ra-main" ${editRooms?`data-evt="click" data-call="${r.archived?'restoreRoom':'openRoomForm'}" data-a1="${r.id}"`:'disabled aria-disabled="true"'} aria-label="${editRooms?(r.archived?'Khôi phục':'Sửa'):'Xem'} phòng ${esc(r.name)}">
              ...
              ${editRooms?`<span class="ra-go" aria-hidden="true">${icon('settings',16)}</span>`:''}
            </button>
            ${r.archived||!editRooms?'':`<div class="ra-extra"><button class="icon-btn" data-evt="click" data-call="openRoomAssets" data-a1="${r.id}">Tài sản</button><button class="icon-btn danger" data-evt="click" data-call="deleteRoom" data-a1="${r.id}">Lưu trữ / Xóa</button></div>`}
...
window.quickRoomMoney=function(id,key,value){if(!requireCan('edit','rooms'))return;const r=getRoom(id);if(!r)return;r[key]=Math.max(0,Number(value||0));saveData();renderPublic();showToast(key==='price'?'Đã cập nhật giá phòng':'Đã cập nhật tiền cọc')}
window.quickRoomStatus=function(id,status){if(!requireCan('edit','rooms'))return;const r=getRoom(id);if(!r)return;const requested=status;r.status=requested;reconcileRoomStatus(id);saveData();renderAdmin();renderPublic();if(requested==='reserved'&&r.status!=='reserved'){showToast('“Đã giữ chỗ” chỉ được suy ra từ phiếu giữ chỗ hoặc hợp đồng nháp còn tiền cọc.');return}if(requested==='occupied'&&r.status!=='occupied'){showToast('“Đã thuê” chỉ được suy ra từ hợp đồng hoặc người ở đang hoạt động.');return}if(requested==='maintenance'&&r.status!=='maintenance'){showToast(`Phòng đang ${r.status==='occupied'?'có người thuê':'được giữ chỗ'} nên không thể chuyển sang bảo trì. Hãy xử lý hợp đồng/giữ chỗ trước.`);return}showToast('Đã đổi trạng thái phòng')}
window.deleteRoom=async function(id){if(!requireCan('edit','rooms'))return;const r=getRoom(id);if(!r)return;
  const openReservation=activeReservationForRoom(id);
  if(openReservation){showToast(`Phòng còn phiếu giữ chỗ của ${openReservation.customerName||'khách'}. Hãy xử lý tiền và đóng phiếu trước khi lưu trữ.`);return}
  const blockingLease=data.leases.find(l=>l.roomId===id&&['draft','active','ending'].includes(l.status));
  if(blockingLease){showToast(`Phòng còn hợp đồng ${leaseStatusLabel(blockingLease.status).toLowerCase()}. Hãy hủy hợp đồng nháp hoặc thực hiện Trả phòng/Thanh lý trước.`);return}
  const activeTenant=data.tenants.find(t=>t.roomId===id&&t.active);
  if(activeTenant){showToast(`Phòng còn người ở hoạt động (${activeTenant.name||'chưa rõ tên'}). Hãy xử lý trả phòng trước khi lưu trữ.`);return}
...
window.restoreRoom=function(id){if(!requireCan('edit','rooms'))return;const r=getRoom(id);if(!r)return;r.archived=false;reconcileRoomStatus(id);saveData();renderAdmin();renderPublic();showToast('Đã khôi phục phòng')}
window.deleteProperty=async function(id){if(!requireCan('edit','properties'))return;const p=getProperty(id);if(!p)return;const rooms=data.rooms.filter(r=>r.propertyId===id);
  const openReservation=data.reservations.find(x=>rooms.some(r=>r.id===x.roomId)&&reservationOpen(x));
  if(openReservation){showToast(`Căn trọ còn phiếu giữ chỗ của ${openReservation.customerName||'khách'}. Hãy xử lý tiền và đóng phiếu trước.`);return}
  const roomIds0=new Set(rooms.map(r=>r.id));
  const blockingLease=data.leases.find(l=>roomIds0.has(l.roomId)&&['draft','active','ending'].includes(l.status));
  if(blockingLease){showToast(`Căn trọ còn hợp đồng ${leaseStatusLabel(blockingLease.status).toLowerCase()}. Hãy xử lý hợp đồng trước khi lưu trữ.`);return}
  const activeTenant=data.tenants.find(t=>roomIds0.has(t.roomId)&&t.active);
  if(activeTenant){showToast(`Căn trọ còn người ở hoạt động (${activeTenant.name||'chưa rõ tên'}). Hãy xử lý trả phòng trước.`);return}
...
window.restoreProperty=function(id){if(!requireCan('edit','properties'))return;const p=getProperty(id);if(!p)return;p.archived=false;saveData();renderAdmin();renderPublic();showToast('Đã khôi phục căn trọ')}
window.openPropertyForm=function(id=null){if(!requireCan(id?'edit':'create','properties'))return;const p=id?getProperty(id):null;document.getElementById('propertyModalTitle').textContent=p?'Sửa căn trọ & ảnh':'Thêm căn trọ';document.getElementById('propertyId').value=p?.id||'';document.getElementById('propertyName').value=p?.name||'';document.getElementById('propertyArea').value=p?.area||'';document.getElementById('propertyAddress').value=p?.address||'';document.getElementById('propertyDescription').value=p?.description||'';document.getElementById('propertyPhone').value=p?.phone||data.settings.managerPhone||'';document.getElementById('propertySlug').value=p?.slug||'';document.getElementById('propertyImages').value='';propertyImageState={existing:[...(p?.imageIds||[])],removed:[],newFiles:[]};renderImageEditor('property');openModal('propertyModal')}
...
window.openTenantForm=function(id=null,roomId=null){if(!requireCan(id?'edit':'create','tenants'))return;if(!data.rooms.length){showToast('Hãy tạo phòng trước.');return}const t=id?getTenant(id):null;document.getElementById('tenantModalTitle').textContent=t?'Sửa hồ sơ người ở':'Thêm hồ sơ người ở';document.getElementById('tenantId').value=t?.id||'';document.getElementById('tenantName').value=t?.name||'';document.getElementById('tenantPhone').value=t?.phone||'';fillTenantRoomSelect(t?.roomId||roomId||data.rooms[0].id);document.getElementById('tenantMoveIn').value=t?.moveInDate||today();document.getElementById('tenantActive').value=String(t?!!t.active:false);const rr=getRoom(t?.roomId||roomId||data.rooms[0].id);document.getElementById('tenantDepositRequired').value=t?.depositRequired??rr?.deposit??0;document.getElementById('tenantDepositPaid').value=t?.depositPaid??0;document.getElementById('tenantNote').value=t?.note||'';const locked=!!(t&&liveLease(leaseForOccupant(t.id)));['tenantRoom','tenantActive','tenantDepositRequired','tenantDepositPaid'].forEach(fid=>document.getElementById(fid).disabled=locked);openModal('tenantModal')}
...
function renderTenants(){const root=document.getElementById('view-tenants');
  ...
  const editTenants=can('edit','tenants'),canPin=ownerManagerOnly(),canRemind=can('create','reminders');
  ...
  if(st.filter==='active')list=list.filter(t=>t.active||reservationActive(reservationForTenant(t.id))||reservationExpired(reservationForTenant(t.id)));
  if(st.filter==='inactive')list=list.filter(t=>!t.active&&!reservationActive(reservationForTenant(t.id))&&!reservationExpired(reservationForTenant(t.id)));
  list.sort((a,b)=>String(a.name).localeCompare(String(b.name),'vi'));
  const {slice,nav}=paginate(list,'tenants');
  root.innerHTML=`<div class="panel"><div class="panel-head"><div><h3>Người ở & tài khoản cư dân</h3><p>Một hợp đồng có thể có nhiều người ở; chỉ một người là đại diện thanh toán. Tài khoản dùng số điện thoại + PIN 6 số.</p></div>${can('create','tenants')?`<button class="btn btn-primary" onclick="openTenantForm()">${icon('plus',15)} Thêm hồ sơ người ở</button>`:''}</div>
  ...
  <div class="table-wrap"><table class="data-table"><thead><tr><th>Người ở</th><th>Điện thoại</th><th>Phòng</th><th>Cọc phải đóng</th><th>Đã nhận</th><th>Còn thiếu</th><th>Tài khoản</th><th></th></tr></thead><tbody>${slice.length?slice.map(t=>{const r=getRoom(t.roomId),res=reservationForTenant(t.id),hr=(reservationActive(res)||reservationExpired(res))?getRoom(res.roomId):null,p=getProperty((hr||r)?.propertyId);
    return `<tr class="${reservationExpired(res)?'row-hold-expired':''}"><td><strong>${esc(t.name)}</strong><br>${tenantStatusBadge(t)}${hr?`<br><span style="color:var(--muted)">Giữ ${esc(hr.name)} · ${esc(res.fromDate)}→${esc(res.untilDate)} · ${money(res.amount)}</span>`:''}</td><td>${esc(t.phone)}</td><td>${esc(p?.name||'')} · ${esc(hr?.name||r?.name||'-')}</td><td>${money(t.depositRequired)}</td><td>${money(t.depositPaid)}</td><td>${money(Math.max(0,t.depositRequired-t.depositPaid))}</td><td>${tenantHasPin(t)?'<span class="badge badge-paid">Đã có PIN</span>':'<span class="badge badge-unpaid">Chưa có PIN</span>'}</td><td><div class="table-actions">${t.phone&&canRemind?`<button class="icon-btn btn-zalo" data-evt="click" data-call="zaloToTenant" data-a1="${esc(t.id)}">${icon('message',13)} Zalo</button>`:''}${canManageHold()?(reservationActive(res)||reservationExpired(res)?`<button class="icon-btn" data-evt="click" data-call="cancelHold" data-a1="${esc(t.id)}">Xử lý giữ chỗ</button>`:!activeLeaseForTenant(t.id)?`<button class="icon-btn" data-evt="click" data-call="openHoldForm" data-a1="${esc(t.id)}">Giữ chỗ</button>`:''):''}${editTenants?`<button class="icon-btn" data-act="tenantEdit" data-id="${esc(t.id)}">Sửa</button><button class="icon-btn danger" data-act="tenantDelete" data-id="${esc(t.id)}">${data.invoices.some(i=>i.tenantId===t.id)?'Lưu trữ':'Xóa'}</button>`:''}${canPin?`<button class="icon-btn" data-act="tenantPin" data-id="${esc(t.id)}">Đặt lại PIN</button>`:''}</div></td></tr>`}).join(''):`<tr><td colspan="8">${data.tenants.length?emptyState('search','Không khớp bộ lọc','Thử từ khóa khác hoặc chuyển bộ lọc về Tất cả.',''):emptyState('users','Chưa có người thuê','Tạo hợp đồng từ CRM khách xem phòng, hoặc thêm trực tiếp tại đây.',can('create','tenants')?`<button class="btn btn-primary" onclick="openTenantForm()">${icon('plus',15)} Thêm người thuê</button>`:'')}</td></tr>`}</tbody></table></div>${nav}</div>`}
...
window.deleteTenant=function(id){
  if(!requireCan('edit','tenants'))return;
  const t=getTenant(id);if(!t)return;
  const roomId=t.roomId;
  const openReservation=data.reservations.find(r=>r.tenantId===id&&reservationOpen(r));
  if(openReservation){showToast('Khách còn phiếu giữ chỗ và tiền cọc chưa xử lý. Hãy đóng phiếu trước khi lưu trữ hồ sơ.');return}
  const currentLink=data.leaseOccupants.find(x=>x.occupantId===id&&!x.leftAt&&data.leases.some(l=>l.id===x.leaseId&&['draft','active','ending'].includes(l.status)));
  const currentLease=currentLink&&getLease(currentLink.leaseId)||data.leases.find(l=>l.primaryTenantId===id&&['draft','active','ending'].includes(l.status));
  if(currentLease){showToast(currentLease.status==='draft'?'Người này đang thuộc hợp đồng nháp. Hãy hủy/xóa hợp đồng nháp trước.':'Người này đang thuộc hợp đồng hiệu lực. Hãy dùng chức năng Trả phòng/Thanh lý; người ở cùng dùng nút “Rời hợp đồng”.');return}
  const hasHistory=data.invoices.some(i=>i.tenantId===id)||data.leaseOccupants.some(x=>x.occupantId===id)||data.reservations.some(r=>r.tenantId===id);
  if(hasHistory){
    if(!confirm('Hồ sơ này đã có lịch sử hợp đồng, hóa đơn hoặc giữ chỗ nên sẽ được LƯU TRỮ (không xóa dữ liệu). Tiếp tục?'))return;
...
function updateUtilityGapNote(roomId,month,previous){
  ...
  if(previous&&previous.month!==prevMonth(month)){
    note.innerHTML=`${icon('alert',15)} Chỉ số gần nhất của phòng này là tháng <strong>${esc(previous.month)}</strong> — đã bỏ trống ${'tháng ở giữa. Đầu kỳ đang lấy theo cuối kỳ '+esc(previous.month)+', hãy kiểm tra lại trước khi lưu.'}`;
  }else{
    note.innerHTML='<strong>Mẹo:</strong> đầu kỳ sẽ tự lấy từ cuối kỳ tháng trước nếu có dữ liệu. Anh chỉ cần kiểm tra và nhập chỉ số cuối tháng.';
  }
}
...
window.openUtilityForm=function(id=null,roomId=null){if(!requireCan(id?'edit':'create','utilityReadings'))return;if(!data.rooms.length){showToast('Chưa có phòng để ghi điện nước.');return}const u=id?getReading(id):null;
  if(u?.status==='final'){showToast('Kỳ này đã chốt. Chủ nhà/quản lý phải mở khóa kèm lý do trước khi sửa.');return}
...
function renderUtilities(){
  const props=scopedProperties();
  const meterCreate=can('create','utilityReadings'),meterEdit=can('edit','utilityReadings');
  const serviceCreate=can('create','serviceDefinitions'),serviceEdit=can('edit','serviceDefinitions');
  const invoiceCreate=can('create','invoices');
  ...
  const boardHtml=`<div class="panel">
    <div class="panel-head"><div><h3>Chốt điện nước tháng</h3><p>Nhập cuối kỳ liên tục cho cả căn trên một màn hình — đầu kỳ tự lấy từ tháng trước, lưu nháp ngay khi nhập</p></div>
      ${can('approve','invoices')?`<button class="btn btn-primary" onclick="openBulkInvoice()">${icon('receipt',16)} Tạo hóa đơn hàng loạt</button>`:''}</div>
    <div class="meter-controls">
      ...
      ${meterEdit?`<button class="btn btn-light" onclick="finalizeMeterMonth()">${icon('key',16)} Chốt kỳ ${meterBoard.month}</button>`:''}
    </div>
    ${rooms.length?rooms.map(r=>meterRowHtml(r,meterBoard.month)).join(''):'<div class="empty">Căn này chưa có phòng đang cho thuê.</div>'}
  </div>
  <div class="panel"><div class="panel-head"><div><h3>Dịch vụ áp dụng cho hợp đồng</h3><p>Wifi, rác, giữ xe… — tính cố định/phòng, theo người, theo số lượng hoặc nhập tay; gắn vào từng hợp đồng trong Chi tiết hợp đồng</p></div>${serviceCreate?`<button class="btn btn-primary" onclick="openServiceForm()">+ Dịch vụ</button>`:''}</div>
  ...
  document.getElementById('view-utilities').innerHTML=boardHtml+`<div class="panel"><div class="panel-head"><div><h3>Lịch sử chỉ số</h3><p>Kỳ đã chốt bị khóa — muốn sửa phải mở khóa kèm lý do</p></div>${meterCreate?`<button class="btn btn-light" onclick="openUtilityForm()">${icon('plus',15)} Ghi chỉ số lẻ</button>`:''}</div>
...
function refreshInvoiceForm(){
  ...
  if(note)note.innerHTML=t
    ?`${icon('receipt',15)} Người thuê trong kỳ: <strong>${esc(t.name)}</strong> · ${esc(t.phone)}${u?'':' · <em>Chưa có chỉ số điện nước tháng này</em>'}${existing?' · <strong>Đang sửa hóa đơn đã có</strong>':''}`
    :`${icon('alert',15)} Phòng này chưa có người thuê đang hoạt động — không thể lập hóa đơn.`;
  ...
  // Ngày thu là ngày trong tháng (1–28), không phải số ngày cộng từ hôm nay.
  // Dùng cùng một hàm với hóa đơn hàng loạt để hai lối phát hành luôn cho cùng kết quả.
  document.getElementById('invoiceDueDate').value=existing?.dueDate||dueDateForMonth(month,Number(lease?.billingDay||data.settings.defaultDueDay||5));
...
window.openInvoiceForm=function(readingId=null){if(!requireCan('create','invoices'))return;if(!data.rooms.length){showToast('Chưa có phòng.');return}
...
window.recordPayment=function(id){if(!requireCan('approve','payments'))return;const i=getInvoice(id);if(!i)return;const t=getTenant(i.tenantId),r=getRoom(i.roomId);const remaining=remainingInvoice(i);
...
document.getElementById('paymentForm').addEventListener('submit',e=>{e.preventDefault();
  if(!requireCan('approve','payments'))return;
...
window.deleteInvoice=function(id){
  if(!requireCan('edit','invoices'))return;
...
/** Chặn ngay tại handler, không chỉ ẩn nút. Nhờ vậy gọi trực tiếp hàm hoặc mở form
 * từ nút cũ cũng không thể tạo dữ liệu cục bộ rồi chờ máy chủ từ chối. */
function requireCan(action,module){if(can(action,module))return true;denyToast();return false}
function ownerManagerOnly(){return !roleIsPending()&&['owner','manager'].includes(currentRole())}
function requireOwnerManager(){if(ownerManagerOnly())return true;denyToast();return false}
function ownerOnly(){return !roleIsPending()&&currentRole()==='owner'}
function requireOwner(){if(ownerOnly())return true;denyToast();return false}
...
window.openPalette=function(){
  ...
  openModal('palModal');setTimeout(()=>inp.focus(),30);
}
window.closePalette=function(){closeModal('palModal')}
...
const uiRenderTimers={};
const uiRenderers={invoices:renderInvoices,tenants:renderTenants,leases:renderLeases,utilities:renderUtilities,appointments:renderAppointments,tickets:renderTicketsAdmin,audit:renderSettings};
function renderUiState(view,restoreSearch){
  const active=document.activeElement;
  const beforeRoot=document.getElementById(view==='audit'?'view-settings':'view-'+view);
  const wasSearch=!!(restoreSearch&&active&&active.matches?.('.tt-search input')&&beforeRoot?.contains(active));
  const pos=wasSearch&&typeof active.selectionStart==='number'?active.selectionStart:null;
  uiRenderers[view]?.();
  const root=document.getElementById(view==='audit'?'view-settings':'view-'+view);
  if(root)applyTableLabels(root);
  if(wasSearch&&root){
    const input=root.querySelector('.tt-search input');
    if(input){input.focus({preventScroll:true});const at=pos==null?input.value.length:Math.min(pos,input.value.length);try{input.setSelectionRange(at,at)}catch(e){}}
  }
}
window.uiSet=function(view,field,value){
  const p={};p[field]=value;if(field!=='page')p.page=1;setUi(view,p);
  clearTimeout(uiRenderTimers[view]);
  if(field==='q'){
    uiRenderTimers[view]=setTimeout(()=>renderUiState(view,true),180);
    return;
  }
  renderUiState(view,false);
}
...
/* ==================================================================
   CRM P2: Yêu cầu mới → Đã liên hệ → Đã hẹn → Đã xem → Giữ chỗ →
   Ký hợp đồng → Không phù hợp. Hợp đồng nháp là chi tiết công việc,
   không phải một tầng riêng làm lệch báo cáo phễu.
   ================================================================== */
const LEAD_STATUS=P2.PIPELINE_LABELS;
const LEAD_SOURCE={website:'Website',facebook:'Facebook',zalo:'Zalo',walkin:'Khách ghé',referral:'Giới thiệu',other:'Khác'};
function leadOpen(a){return P2.isOpenAppointment(a)}
...
window.leadSetStatus=function(id,st){
  if(!requireCan('edit','appointments'))return;
  const a=data.appointments.find(x=>x.id===id);if(!a)return;
  if(a.status===st)return;
  const res=reservationForAppointment(id);
  if(st==='reserved'&&!reservationOpen(res)){
    showToast('Không thể chọn “Đang giữ chỗ” bằng tay. Hãy bấm nút Giữ chỗ để ghi khách, hạn và sổ cọc.');renderAppointments();return;
  }
  if(reservationOpen(res)&&st!=='reserved'){
    showToast('Khách đang có phiếu giữ chỗ. Hãy xử lý giữ chỗ và tiền cọc trước khi đổi trạng thái.');renderAppointments();return;
  }
  if(st==='converted'){showToast('Trạng thái “Đã ký HĐ” chỉ được tạo khi xác nhận ký trên hợp đồng.');renderAppointments();return}
  leadLog(a,'Chuyển trạng thái: '+(LEAD_STATUS[a.status]||a.status)+' → '+(LEAD_STATUS[st]||st),'status');
  a.status=st;
  saveData();renderAppointments();
}
window.leadSetSource=function(id,src){if(!requireCan('edit','appointments'))return;const a=data.appointments.find(x=>x.id===id);if(!a)return;a.source=src;saveData()}
...
window.leadReschedule=function(id){
  ...
  const busy=new Set(data.appointments.filter(x=>P2.appointmentClash([x],{roomId:a.roomId,date:a.date,time:x.time},id)).map(x=>x.time));
...
window.refreshRescheduleSlots=function(){
  ...
  const busy=new Set(data.appointments.filter(x=>P2.appointmentClash([x],{roomId:a.roomId,date,time:x.time},id)).map(x=>x.time));
...
document.getElementById('rescheduleForm').addEventListener('submit',async e=>{
  ...
  const clash=P2.appointmentClash(data.appointments,{roomId:a.roomId,date,time},a.id);
  if(clash){showToast('Khung giờ này đã có khách khác. Chọn giờ khác giúp anh.');return}
  const btn=e.submitter;setBtnBusy(btn,true,'Đang lưu…');
  try{
    if(Sync.isOn()&&Sync.isAdmin()){
      const out=await Sync.rescheduleAppointment({appointmentId:a.id,date,time});
      Sync.applyActionResult(out);
    }else{
      leadLog(a,`Đổi lịch: ${a.date} ${a.time} → ${date} ${time}`,'reschedule');
      a.date=date;a.time=time;if(['new','contacted'].includes(a.status))a.status='appointment_confirmed';saveData();
    }
    closeModal('rescheduleModal');renderAppointments();showToast('Đã đổi lịch hẹn và kiểm tra không trùng phòng/giờ.');
  }catch(err){showToast('Chưa đổi được lịch: '+(err.message||err))}
  finally{setBtnBusy(btn,false)}
});
/* ==================================================================
   v4.4 P0 — MỘT NGUỒN GIỮ CHỖ DUY NHẤT: data.reservations
   CRM và hồ sơ người thuê chỉ là hai lối mở cùng một nghiệp vụ. Mỗi lần
   nhận tiền luôn tạo một bút toán append-only trong depositLedger.
   ================================================================== */
function activeLeaseForTenant(tenantId){
  return data.leases.find(l=>liveLease(l)&&l.primaryTenantId===tenantId);
}
/** Trạng thái người thuê để hiển thị thống nhất mọi nơi. */
function tenantStatus(t){
  if(!t)return {key:'unknown',label:'Không xác định',cls:'unpaid'};
  const res=reservationForTenant(t.id);
  if(reservationActive(res))return {key:'holding',label:'Đang đặt cọc giữ chỗ',cls:'partial'};
  if(reservationExpired(res))return {key:'hold_expired',label:'Giữ chỗ đã hết hạn',cls:'overdue'};
  if(activeLeaseForTenant(t.id))return {key:'renting',label:'Đang thuê',cls:'paid'};
  if(t.active)return {key:'waiting',label:'Chưa nhận phòng',cls:'unpaid'};
  return {key:'left',label:'Đã trả phòng',cls:'unpaid'};
}
function tenantStatusBadge(t){
  const st=tenantStatus(t);
  return `<span class="badge badge-${st.cls}">${st.label}</span>`;
}
/** Giữ chỗ gắn với TIỀN CỌC thật nên chỉ Chủ nhà / Quản lý được đặt và hủy.
 * Máy chủ kiểm tra lại quyền, phòng, ngày, số tiền và trùng giữ chỗ. */
function canManageHold(){return ['owner','manager'].includes(currentRole())}
function denyHold(){
  showToast('Chỉ Chủ nhà hoặc Quản lý mới đặt/hủy giữ chỗ được, vì giữ chỗ đi kèm tiền cọc thật.');
}
function reservationDraft(payload){
  const id=payload.id||uid('res');
  return {id,roomId:payload.roomId,sourceType:payload.sourceType,sourceId:payload.sourceId,
    appointmentId:payload.appointmentId||'',tenantId:payload.tenantId||'',customerName:payload.customerName||'',customerPhone:payload.customerPhone||'',
    fromDate:payload.fromDate,untilDate:payload.untilDate,amount:Number(payload.amount||0),paymentMethod:payload.paymentMethod||'cash',
    paymentReference:payload.paymentReference||'',note:payload.note||'',status:'active',depositEntryId:payload.depositEntryId||uid('dep'),
    leaseId:'',cancelledAt:'',cancelReason:'',createdBy:currentStaffName(),createdAt:new Date().toISOString()};
}
function validateReservationDraft(res){
  const room=getRoom(res.roomId);
  if(!room||room.archived)return 'Phòng không tồn tại hoặc đã lưu trữ.';
  if(room.status==='maintenance')return 'Phòng đang bảo trì, không thể giữ chỗ.';
  if(activeLeaseForRoom(room.id)||room.status==='occupied')return 'Phòng đang có người thuê, không thể giữ chỗ.';
  const unresolved=activeReservationForRoom(room.id,'','',res.id);
  if(unresolved)return `Phòng còn phiếu giữ chỗ của ${unresolved.customerName||'khách khác'} chưa xử lý.`;
  if(!res.fromDate||!res.untilDate||res.untilDate<res.fromDate)return 'Khoảng ngày giữ chỗ chưa hợp lệ.';
  if(!(Number(res.amount)>0))return 'Tiền giữ chỗ phải lớn hơn 0.';
  const other=activeReservationForRoom(room.id,res.fromDate,res.untilDate,res.id);
  if(other)return `Phòng đang được ${other.customerName||'khách khác'} giữ đến ${other.untilDate}.`;
  return '';
}
function applyReservationCreateLocal(payload){
  const res=reservationDraft(payload),error=validateReservationDraft(res);
  if(error)throw new Error(error);
  data.reservations.push(res);
  data.depositLedger.push({id:res.depositEntryId,leaseId:'',reservationId:res.id,roomId:res.roomId,tenantId:res.tenantId,
    appointmentId:res.appointmentId,type:'collect',amount:res.amount,at:today(),method:res.paymentMethod,reference:res.paymentReference,
    note:'Thu tiền giữ chỗ'+(res.note?' — '+res.note:''),createdBy:currentStaffName(),createdAt:new Date().toISOString()});
  const room=getRoom(res.roomId);if(room&&room.status==='available')room.status='reserved';
  if(res.appointmentId){const a=data.appointments.find(x=>x.id===res.appointmentId);if(a){a.status='reserved';a.reserveAmount=res.amount;a.reserveUntil=res.untilDate;leadLog(a,`Giữ chỗ ${money(res.amount)} đến hết ${res.untilDate}`,'reserve')}}
  auditLocal('create','reservations',res.id,null,{roomId:res.roomId,amount:res.amount,untilDate:res.untilDate});
  return res;
}
async function createReservation(payload,modalId,button){
  const draft=reservationDraft(payload),error=validateReservationDraft(draft);
  if(error){showToast(error);return null}
  if(Sync.isOn()&&!Sync.isAdmin()){showToast('Thiết bị chưa đăng nhập quyền quản lý.');return null}
  setBtnBusy(button,true,'Đang ghi sổ…');
  try{
    let res;
    if(Sync.isOn()&&Sync.isAdmin()){
      const out=await Sync.createReservation(draft);
      Sync.applyActionResult(out);res=getReservation(draft.id);
    }else{
      res=applyReservationCreateLocal(draft);saveData();
    }
    closeModal(modalId);renderAdmin();renderPublic();
    showToast(`Đã giữ chỗ và ghi sổ cọc ${money(draft.amount)}.`);
    return res;
  }catch(err){showToast('Chưa giữ chỗ được: '+(err.message||err));return null}
  finally{setBtnBusy(button,false)}
}
function applyReservationCancelLocal(res,resolution,status,reason){
  const balance=Math.max(0,reservationBalance(res.id));
  if(balance>0)data.depositLedger.push({id:uid('dep'),leaseId:'',reservationId:res.id,roomId:res.roomId,tenantId:res.tenantId,
    appointmentId:res.appointmentId,type:resolution==='refund'?'refund':'deduct',amount:balance,at:today(),method:resolution==='refund'?(res.paymentMethod||'cash'):'',reference:'',
    note:resolution==='refund'?'Hoàn tiền khi hủy giữ chỗ':'Giữ lại tiền do khách bỏ cọc',createdBy:currentStaffName(),createdAt:new Date().toISOString()});
  res.status=status;res.cancelledAt=new Date().toISOString();res.cancelReason=reason||'';
  if(res.appointmentId){const a=data.appointments.find(x=>x.id===res.appointmentId);if(a){leadLog(a,status==='expired'?'Giữ chỗ hết hạn — đã xử lý tiền':'Đã hủy giữ chỗ — đã xử lý tiền','release');a.status='lost'}}
  reconcileRoomStatus(res.roomId);
  auditLocal('update','reservations',res.id,{status:'active'},{status:res.status,resolution});
}
async function cancelReservationFlow(res,status='cancelled'){
  if(!res||(!reservationActive(res)&&!reservationExpired(res))){showToast('Phiếu giữ chỗ này đã được xử lý.');return}
  if(!canManageHold()){denyHold();return}
  const balance=Math.max(0,reservationBalance(res.id));
  const answer=prompt(`Còn ${money(balance)} đang giữ. Nhập HOAN để hoàn khách hoặc GIU để ghi nhận khách bỏ cọc.`,`HOAN`);
  if(answer===null)return;
  const key=String(answer).trim().toUpperCase();
  if(!['HOAN','GIU'].includes(key)){showToast('Chỉ nhập HOAN hoặc GIU.');return}
  const resolution=key==='HOAN'?'refund':'forfeit';
  const reason=prompt('Lý do xử lý giữ chỗ:',status==='expired'?'Hết hạn giữ chỗ':'Khách hủy giữ chỗ');
  if(reason===null)return;
  if(!confirm(`${resolution==='refund'?'Hoàn':'Giữ lại'} ${money(balance)} và đóng phiếu giữ chỗ của ${res.customerName}?`))return;
  try{
    if(Sync.isOn()&&Sync.isAdmin()){
      const out=await Sync.cancelReservation({reservationId:res.id,resolution,status,reason:reason.trim()});
      Sync.applyActionResult(out);
    }else{
      applyReservationCancelLocal(res,resolution,status,reason.trim());saveData();
    }
    renderAdmin();renderPublic();showToast('Đã đóng giữ chỗ và ghi đầy đủ xử lý tiền vào sổ cọc.');
  }catch(err){showToast('Chưa xử lý được giữ chỗ: '+(err.message||err))}
}
window.openHoldForm=function(tenantId){
  const t=getTenant(tenantId);if(!t)return;
  if(!canManageHold()){denyHold();return}
  if(activeLeaseForTenant(t.id)){showToast('Người này đang có hợp đồng hiệu lực, không thể tạo thêm giữ chỗ từ hồ sơ người thuê.');return}
  const current=reservationForTenant(t.id);
  if(reservationActive(current)||reservationExpired(current)){showToast('Người này đã có một phiếu giữ chỗ chưa xử lý.');return}
  document.getElementById('holdTenantId').value=t.id;
  document.getElementById('holdSummary').innerHTML=`<h4>${esc(t.name)}</h4><p>SĐT ${esc(t.phone||'—')}</p>`;
  const sel=document.getElementById('holdRoom');
  const rooms=visibleRooms().filter(r=>!r.archived&&r.status==='available'&&!activeReservationForRoom(r.id));
  sel.innerHTML=rooms.map(r=>{const p=getProperty(r.propertyId);
    return `<option value="${r.id}">${esc(p?.name||'')} · ${esc(r.name)} — cọc niêm yết ${money(r.deposit)}</option>`}).join('')
    ||'<option value="">Không còn phòng trống</option>';
  document.getElementById('holdFrom').value=today();
  document.getElementById('holdFrom').min=today();
  document.getElementById('holdUntil').value=nextDayISO(7);
  document.getElementById('holdUntil').min=today();
  setMoneyInput('holdAmount',Number(getRoom(sel.value)?.deposit||0));
  document.getElementById('holdMethod').value='cash';document.getElementById('holdReference').value='';document.getElementById('holdNote').value='';
  openModal('holdModal');
}
document.getElementById('holdRoom')?.addEventListener('change',e=>setMoneyInput('holdAmount',Number(getRoom(e.target.value)?.deposit||0)));
document.getElementById('holdForm')?.addEventListener('submit',async e=>{
  e.preventDefault();
  if(!canManageHold()){denyHold();return}
  const t=getTenant(document.getElementById('holdTenantId').value);if(!t)return;
  await createReservation({roomId:document.getElementById('holdRoom').value,sourceType:'tenant',sourceId:t.id,tenantId:t.id,
    customerName:t.name,customerPhone:t.phone,fromDate:document.getElementById('holdFrom').value,untilDate:document.getElementById('holdUntil').value,
    amount:Math.max(0,readMoneyInput('holdAmount')),paymentMethod:document.getElementById('holdMethod').value,
    paymentReference:document.getElementById('holdReference').value.trim(),note:document.getElementById('holdNote').value.trim()},'holdModal',e.submitter);
});
window.cancelHold=function(tenantId){
  const res=reservationForTenant(tenantId);if(!res){showToast('Không tìm thấy phiếu giữ chỗ.');return}
  cancelReservationFlow(res,reservationExpired(res)?'expired':'cancelled');
}
window.leadReserve=function(id){
  const a=data.appointments.find(x=>x.id===id);if(!a)return;
  if(!canManageHold()){denyHold();return}
  const r=getRoom(a.roomId);
  const existing=reservationForAppointment(id);
  if(reservationActive(existing)||reservationExpired(existing)){showToast('Khách này đã có phiếu giữ chỗ chưa xử lý.');return}
  if(!r||r.status!=='available'||activeLeaseForRoom(r.id)||activeReservationForRoom(r.id)){showToast('Phòng không còn trống để giữ chỗ.');return}
  ...
  document.getElementById('rsvFrom').value=today();document.getElementById('rsvFrom').min=today();
  setMoneyInput('rsvAmount',Math.min(500000,Number(r?.deposit||500000)));
  document.getElementById('rsvUntil').value=nextDayISO(3);
  document.getElementById('rsvUntil').min=today();
  document.getElementById('rsvMethod').value='cash';document.getElementById('rsvReference').value='';
...
document.getElementById('reserveForm').addEventListener('submit',async e=>{
  ...
  await createReservation({roomId:a.roomId,sourceType:'appointment',sourceId:a.id,appointmentId:a.id,customerName:a.customerName,
    customerPhone:a.customerPhone,fromDate:document.getElementById('rsvFrom').value,untilDate:document.getElementById('rsvUntil').value,
    amount:readMoneyInput('rsvAmount'),paymentMethod:document.getElementById('rsvMethod').value,
    paymentReference:document.getElementById('rsvReference').value.trim(),note:'Giữ chỗ từ CRM'},'reserveModal',e.submitter);
});
/* Hết hạn giữ chỗ: quản lý XÁC NHẬN mới trả phòng về trạng thái phù hợp */
window.releaseExpiredHold=function(id){
  const a=data.appointments.find(x=>x.id===id);if(!a)return;
  const res=reservationForAppointment(id);if(!res){showToast('Không tìm thấy phiếu giữ chỗ.');return}
  cancelReservationFlow(res,'expired');
}
...
window.convertLeadToLease=function(id){
  ...
  if(reservationBalance(reservation?.id)>0)setMoneyInput('leaseDepositPaid',reservationBalance(reservation.id));
  document.getElementById('leaseSnapshotNote').innerHTML+=`<div class="smart-note">Đang chuyển từ lead <strong>${esc(a.customerName)} · ${esc(a.customerPhone)}</strong>${reservation?` — tiền giữ chỗ ${money(reservationBalance(reservation.id))} đã nằm trong sổ cọc và sẽ liên kết với hợp đồng`:''}. Lưu xong lead ở trạng thái "Soạn hợp đồng"; chỉ chuyển "Đã ký" khi bấm xác nhận ký.</div>`;
...
function completeLeadConversion(lease){
  ...
  const reservation=reservationForAppointment(pendingLeadId);
  pendingLeadId='';
  if(!a)return;
  a.status='viewed';a.convertedLeaseId=lease.id;
  if(reservation){reservation.status='converted';reservation.leaseId=lease.id}
  leadLog(a,'Đã tạo hợp đồng nháp '+lease.id,'convert');
}
function renderAppointments(){
  const st=ui('appointments');
  const editAppointments=can('edit','appointments'),canHold=canManageHold(),canLease=can('create','leases');
  if(!st.p2Ready){st.filter='open';st.p2Ready=true;setUi('appointments',st)}
  const sorted=[...data.appointments].sort((a,b)=>(a.date+a.time).localeCompare(b.date+b.time));
  const openLeads=sorted.filter(leadOpen);
  const expiredHolds=sorted.filter(a=>reservationExpired(reservationForAppointment(a.id)));
  const shown=P2.filterAppointments(sorted,{q:st.q,status:st.filter,from:st.from||'',to:st.to||'',roomName:id=>{const r=getRoom(id),p=getProperty(r?.propertyId);return `${p?.name||''} ${r?.name||''}`}});
  const {slice,nav}=paginate(shown,'appointments',10);
  const funnel=P2.PIPELINE.map(key=>({key,label:LEAD_STATUS[key],count:sorted.filter(a=>a.status===key).length}));
  ...
  ${expiredHolds.length?`<div class="panel" style="border:2px solid #C9A86A"><div class="panel-head"><div><h3>${icon('alert',16)} Hết hạn giữ chỗ (${expiredHolds.length})</h3><p>Khách chưa ký sau hạn giữ — xác nhận để trả phòng về trạng thái phù hợp</p></div></div>
    ${expiredHolds.map(a=>{const res=reservationForAppointment(a.id),r=getRoom(res?.roomId||a.roomId);return `<div class="tk-row"><div class="tk-main"><strong>${esc(a.customerName)} · ${esc(r?.name||'')}</strong><small>Giữ ${money(res?.amount)} đến ${esc(res?.untilDate)} — đã quá hạn</small></div><div class="table-actions">${canHold?`<button class="btn btn-primary" data-act="leadRelease" data-id="${esc(a.id)}">Xử lý tiền & trả phòng</button>`:''}${canLease?`<button class="icon-btn" data-act="leadConvert" data-id="${esc(a.id)}">Vẫn lập HĐ</button>`:''}</div></div>`}).join('')}</div>`:''}
  <div class="panel"><div class="panel-head"><div><h3>Khách xem phòng (CRM)</h3><p>${openLeads.length} khách đang theo dõi · phễu 7 bước thống nhất từ yêu cầu tới kết quả</p></div></div>
  <div class="crm-funnel" aria-label="Phễu khách xem phòng">${funnel.map(x=>`<button type="button" class="${st.filter===x.key?'active':''}" onclick="uiSet('appointments','filter','${x.key}')"><span>${esc(x.label)}</span><strong>${x.count}</strong></button>`).join('')}</div>
  <div class="table-tools crm-tools">
    <span class="tt-search">${icon('search',15)}<input value="${esc(st.q)}" placeholder="Tìm tên, SĐT, phòng, ghi chú…" aria-label="Tìm lịch hẹn" oninput="uiSet('appointments','q',this.value)"></span>
    <select aria-label="Lọc trạng thái" onchange="uiSet('appointments','filter',this.value)">
      <option value="open" ${st.filter==='open'?'selected':''}>Đang theo dõi</option>
      <option value="all" ${st.filter==='all'?'selected':''}>Tất cả</option>
      ${P2.PIPELINE.map(k=>`<option value="${k}" ${st.filter===k?'selected':''}>${LEAD_STATUS[k]}</option>`).join('')}
    </select>
    <label class="date-filter">Từ ngày<input type="date" value="${esc(st.from||'')}" onchange="uiSet('appointments','from',this.value)"></label>
    <label class="date-filter">Đến ngày<input type="date" value="${esc(st.to||'')}" onchange="uiSet('appointments','to',this.value)"></label>
  </div>
  ${slice.length?slice.map(a=>{
    const res=reservationForAppointment(a.id),r=getRoom(a.roomId),p=getProperty(r?.propertyId);
    const d=new Date(`${a.date}T00:00:00`);
    const care=(a.careLog||[]).slice(-3).reverse();
    const draft=a.convertedLeaseId?getLease(a.convertedLeaseId):null;
    return `<div class="lead-card ${a.status==='reserved'?'lead-reserved':''}">
      ...
      <div class="lead-info">
        <strong>${esc(a.customerName)} · ${esc(a.customerPhone)}</strong> ${leadBadge(a.status)} ${draft&&draft.status==='draft'?'<span class="badge badge-partial">Đang soạn HĐ</span>':''}
        <span>${esc(p?.name||'')} · ${esc(r?.name||'-')} · ${esc(a.time)} ngày ${esc(a.date)}</span>
        <small>Nguồn: <select class="select-inline" data-chg="leadSource" data-id="${esc(a.id)}" ${editAppointments?'':'disabled'}>${Object.entries(LEAD_SOURCE).map(([k,v])=>`<option value="${k}" ${a.source===k?'selected':''}>${v}</option>`).join('')}</select>
        ${res?` · Giữ ${money(res.amount)} đến ${esc(res.untilDate)} · sổ cọc ${money(reservationBalance(res.id))}`:''}${a.note?` · ${esc(a.note)}`:''}</small>
      ...
      <div class="lead-actions">
        <select class="select-small" data-chg="leadStatus" data-id="${esc(a.id)}" aria-label="Trạng thái lead" ${editAppointments?'':'disabled'}>${P2.PIPELINE.map(k=>`<option value="${k}" ${a.status===k?'selected':''}>${LEAD_STATUS[k]}</option>`).join('')}</select>
        <div class="table-actions">
          ${editAppointments?`<button class="icon-btn" data-act="leadCare" data-id="${esc(a.id)}" aria-label="Ghi chăm sóc">${icon('message',14)} Chăm sóc</button>`:''}
          ${editAppointments&&leadOpen(a)&&!reservationOpen(res)?`<button class="icon-btn" data-act="leadReschedule" data-id="${esc(a.id)}">${icon('calendar',14)} Đổi lịch</button>`:''}
          ${canHold&&leadOpen(a)&&!reservationActive(res)&&!reservationExpired(res)?`<button class="icon-btn" data-act="leadReserve" data-id="${esc(a.id)}">Giữ chỗ</button>`:''}
          ${canLease&&leadOpen(a)&&!a.convertedLeaseId?`<button class="icon-btn" data-act="leadConvert" data-id="${esc(a.id)}">${icon('filetext',14)} Lập hợp đồng</button>`:''}
          ${a.convertedLeaseId?`<button class="icon-btn" data-act="leaseDetail" data-id="${esc(a.convertedLeaseId)}">Xem HĐ</button>`:''}
          ${editAppointments?`<button class="icon-btn danger" data-act="leadDelete" data-id="${esc(a.id)}">Xóa</button>`:''}
        </div>
      </div>
    </div>`}).join(''):emptyState('search','Không có lịch hẹn phù hợp','Thử xóa từ khóa, đổi khoảng ngày hoặc chuyển bộ lọc về Tất cả.','')}
  ${nav}
  </div>`;
}
window.updateAppointmentStatus=function(id,status){if(!requireCan('edit','appointments'))return;const a=data.appointments.find(x=>x.id===id);if(!a)return;a.status=status;saveData();renderAdmin();showToast('Đã cập nhật lịch hẹn')}
window.deleteAppointment=function(id){if(!requireCan('edit','appointments'))return;const res=reservationForAppointment(id);if(reservationOpen(res)){showToast('Khách còn phiếu giữ chỗ và tiền cọc. Hãy xử lý giữ chỗ trước khi xóa.');return}if(res){showToast('Khách này đã có lịch sử giữ chỗ nên không thể xóa; hãy chuyển trạng thái “Không thành” để giữ đối soát.');return}if(!confirm('Xóa lịch hẹn này?'))return;data.appointments=data.appointments.filter(a=>a.id!==id);saveData();renderAdmin();showToast('Đã xóa lịch hẹn')}
...
window.zaloToTenant=async function(tenantId,message){
  if(!requireCan('create','reminders'))return;
...
function renderTicketsAdmin(){
  ...
  const editTickets=can('edit','maintenanceTickets'),createNotifications=can('create','notifications'),editNotifications=can('edit','notifications');
  ...
  ${list.length?list.map(k=>{const t=getTenant(k.tenantId),r=getRoom(k.roomId);
    return `<div class="tk-row ${k.priority==='urgent'?'tk-urgent':''}">
      <div class="tk-main rs-click" data-evt="click" data-call="openTicketDetail" data-a1="${k.id}">
        ...
        <small>${esc(r?.name||'?')} · ${esc(t?.name||'?')} · ${esc((k.createdAt||'').slice(0,16).replace('T',' '))}${k.assigneeId?` · ${icon('users',12)} ${esc(k.assigneeId)}`:''}${(k.imageIds||[]).length?` · ${icon('camera',12)}${k.imageIds.length}`:''}</small>
      </div>
      <div class="table-actions">
        ${editTickets&&k.status==='new'?`<button class="icon-btn" data-evt="click" data-call="ticketAction" data-a1="${k.id}" data-a2="received">Tiếp nhận</button>`:''}
        ${editTickets&&ticketOpen(k)?`<button class="icon-btn" data-evt="click" data-call="ticketAction" data-a1="${k.id}" data-a2="assign">Phân công</button>`:''}
        ${editTickets&&['received','waiting'].includes(k.status)?`<button class="icon-btn" data-evt="click" data-call="ticketAction" data-a1="${k.id}" data-a2="progress">Bắt đầu</button>`:''}
        ${editTickets&&k.status==='in_progress'?`<button class="icon-btn" data-evt="click" data-call="ticketAction" data-a1="${k.id}" data-a2="waiting">Chờ</button>`:''}
        ${editTickets&&ticketOpen(k)?`<button class="icon-btn" data-evt="click" data-call="ticketAction" data-a1="${k.id}" data-a2="done">✔ Xong</button><button class="icon-btn danger" data-evt="click" data-call="ticketAction" data-a1="${k.id}" data-a2="cancel">Hủy</button>`:''}
      </div>
  ...
  <div class="panel"><div class="panel-head"><div><h3>Gửi thông báo cho cư dân</h3><p>Thông báo chung (mọi cư dân) hoặc riêng một người — hiện trong cổng cư dân, mục Thông báo</p></div></div>
    ...
    </div>`:'<p class="muted-text">Vai trò hiện tại chỉ được xem thông báo đã gửi.</p>'}
    ${data.notifications.filter(n=>['general','maintenance'].includes(n.kind)).slice(-5).reverse().map(n=>`<div class="rem-log">${noticeIcon(n.kind)} ${esc(n.title)} · ${n.tenantId?esc(getTenant(n.tenantId)?.name||''):'tất cả'} · ${esc(String(n.createdAt).slice(0,16).replace('T',' '))} ${n.tenantId&&getTenant(n.tenantId)?.phone&&can('create','reminders')?`<button class="icon-btn btn-zalo" data-evt="click" data-call="zaloSendNotice" data-a1="${n.id}">${icon('message',13)} Gửi Zalo</button>`:''}${editNotifications?`<button class="icon-btn danger" data-evt="click" data-call="deleteNotice" data-a1="${n.id}">Xóa</button>`:''}</div>`).join('')}
  </div>`;
}
window.sendAdminNotice=function(){
  if(!requireCan('create','notifications'))return;
...
window.deleteNotice=function(id){
  if(!requireCan('edit','notifications'))return;
...
window.meterInput=function(roomId,field,value){
  ...
  if(!requireCan(rec?'edit':'create','utilityReadings'))return;
...
function meterRowHtml(room,month){
  ...
  const writable=rec?can('edit','utilityReadings'):can('create','utilityReadings');
  const locked=rec?.status==='final'||!writable;
  ...
  return `<div class="meter-row ${locked?'meter-locked':''}" data-meter-room="${room.id}">
    ...
    <div class="meter-foot">
      ...
      <span class="table-actions">
        ${writable?`<button type="button" class="icon-btn" data-evt="click" data-call="attachMeterPhoto" data-a1="${room.id}">${icon('camera',14)} Ảnh công tơ${photoCount?` (${photoCount})`:''}</button>`:''}
        ${photoCount?`<button type="button" class="icon-btn" data-evt="click" data-call="openMeterPhotos" data-a1="${rec.id}">Xem ảnh</button>`:''}
        ${rec?.status==='final'&&ownerManagerOnly()?`<button type="button" class="icon-btn danger" data-evt="click" data-call="unlockReading" data-a1="${rec.id}">Mở khóa</button>`:''}
      </span>
    </div>
  </div>`;
}
...
window.attachMeterPhoto=function(roomId){
  const existing=findReading(roomId,meterBoard.month);
  if(!requireCan(existing?'edit':'create','utilityReadings'))return;
  const month=meterBoard.month;
  const input=document.createElement('input');
...
window.renderBulkPreview=function(){
  ...
  body.innerHTML=`
    ...
    ${bulkState.skipped.length?`<div class="bulk-skips"><h4>Bỏ qua ${bulkState.skipped.length} phòng</h4>${bulkState.skipped.map(x=>`<div class="bulk-skip">${icon('alert',14)} <strong>${esc(x.room?.name||'?')}</strong> — ${esc(x.reason)}</div>`).join('')}</div>`:''}`;
...
window.bulkAdjust=function(idx,field,value){
  if(!requireCan('approve','invoices'))return;
...
window.openServiceForm=function(id=null){
  if(!requireCan(id?'edit':'create','serviceDefinitions'))return;
...
document.getElementById('serviceForm').addEventListener('submit',e=>{
  ...
  saveData();closeModal('serviceModal');renderAdmin();
  showToast('Đã lưu dịch vụ');
});
window.archiveService=function(id){
  if(!requireCan('edit','serviceDefinitions'))return;
...
window.openLeaseServiceForm=function(leaseId){
  if(!requireCan('create','leaseServices'))return;
...
window.endLeaseService=function(lsId){
  if(!requireCan('edit','leaseServices'))return;
...
window.openDepositForm=function(leaseId,type){
  if(!requireCan('create','depositLedger'))return;
  const l=getLease(leaseId);if(!l)return;
  ...
  document.getElementById('payHistoryBody').innerHTML=`
    ...
    ${txs.length?txs.map(p=>`
      <div class="pay-tx ${p.kind==='reversal'?'pay-rev':''} ${p.reversedAt?'pay-reversed':''}">
        <div><strong>${icon(p.kind==='reversal'?'back':'receipt',14)} ${p.kind==='reversal'?'Đảo':'Thu'} ${money(Math.abs(p.amount))}</strong> · ${esc(p.paidAt)} · ${({cash:'Tiền mặt',bank:'Chuyển khoản',other:'Khác'})[p.method]||esc(p.method)}
        ...
        <div class="table-actions">
          ${p.kind==='payment'&&!p.reversedAt&&can('edit','payments')?`<button class="icon-btn danger" data-evt="click" data-call="doReverse" data-a1="${p.id}">Đảo giao dịch</button>`:''}
...
window.doReverse=function(txId){
  if(!requireCan('edit','payments'))return;
...
function vietQrUrl(amount,addInfo,settings){
...
function buildInvoiceDocHtml(inv,ctx){
  const t=ctx.tenant,r=ctx.room,p=ctx.property,st=ctx.settings||data.settings||{};
  ...
  const qr=vietQrUrl(remaining,code,st);
...
window.openInvoicePdf=function(invoiceId){
  ...
  if(residentSession){
    ...
    ctx={tenant:residentSession.tenant,room:residentSession.room,property:residentSession.property,settings:residentSession.settings};
  }else{
...
window.openReminder=function(invoiceId){
  ...
  document.getElementById('remHistory').innerHTML=hist.length?'<h4>Đã gửi</h4>'+hist.map(h=>`<div class="rem-log">${icon('message',14)} ${esc(h.sentAt.slice(0,16).replace('T',' '))} · ${({before:'Trước hạn',due:'Đến hạn',overdue:'Quá hạn'})[h.kind]||h.kind} · ${esc(h.channel)}</div>`).join(''):'';
...
function leaseRoomOptionsHtml(selectedId){
  const sourceReservation=pendingLeadId?reservationForAppointment(pendingLeadId):null;
  return visibleRooms().filter(r=>r.id===selectedId||(
    !r.archived&&r.status!=='maintenance'&&!activeLeaseForRoom(r.id)&&!draftLeaseForRoom(r.id)&&
    (!activeReservationForRoom(r.id)||sourceReservation?.roomId===r.id)
  )).map(r=>{const p=getProperty(r.propertyId);return `<option value="${r.id}" ${r.id===selectedId?'selected':''}>${esc(p?.name||'')} · ${esc(r.name)} — ${money(r.price)}/tháng</option>`}).join('');
}

/** Lấy hồ sơ người ở hoặc tạo một bản nháp chưa chèn vào data.
 * Chỉ commit sau khi toàn bộ form hợp đồng/sức chứa đã hợp lệ để không sinh hồ sơ rác. */
function occupantFromPicker(selectId,nameId,phoneId){
  ...
  // Hồ sơ mới trong HĐ nháp chưa phải là người đang ở; chỉ bật active khi Nhận phòng.
  const t={id:uid('t'),name,phone,pin:'',roomId:'',moveInDate:'',active:false,moveOutDate:'',
    depositRequired:0,depositPaid:0,note:'',_newOccupantDraft:true};
  return t;
}
function commitOccupantDraft(t){
  if(t?._newOccupantDraft){delete t._newOccupantDraft;data.tenants.push(t)}
  return t;
}
...
window.openLeaseForm=function(id=null,roomId=null){
  if(!requireCan(id?'edit':'create','leases'))return;
  ...
  const chosenId=l?.roomId||roomId||firstRoom.id,chosen=getRoom(chosenId),sourceReservation=pendingLeadId?reservationForAppointment(pendingLeadId):null;
  if(!l&&(chosen?.status==='maintenance'||activeLeaseForRoom(chosenId)||draftLeaseForRoom(chosenId)||(activeReservationForRoom(chosenId)&&sourceReservation?.roomId!==chosenId))){showToast('Phòng này không còn trống để tạo hợp đồng. Hãy chọn phòng khác hoặc xử lý giữ chỗ trước.');return}
  document.getElementById('leaseRoom').innerHTML=leaseRoomOptionsHtml(chosenId);
  ...
  setMoneyInput('leaseDepositPaid',l?leaseDepositCollected(l.id):0);
  ...
  document.getElementById('leaseSnapshotNote').innerHTML=
    '<strong>Lưu ý:</strong> giá thuê và tiền cọc được <strong>chốt trên hợp đồng</strong>: sau này đổi giá niêm yết của phòng sẽ KHÔNG làm thay đổi hợp đồng này.';
...
document.getElementById('leaseForm').addEventListener('submit',e=>{
  e.preventDefault();
  const editingLeaseId=document.getElementById('leaseId').value;
  if(!requireCan(editingLeaseId?'edit':'create','leases'))return;
  ...
  let lease;
  if(l){
    const before=leaseDepositCollected(l.id);
    if(Number(values.depositPaid)<before){
      showToast('Cọc đã ghi sổ không được giảm bằng cách sửa hợp đồng. Hãy dùng “Hoàn cọc” để có lịch sử đối chiếu.');return;
    }
    Object.assign(l,values);lease=l;
    const diff=Number(values.depositPaid||0)-before;
    if(diff>0)addDepositEntry(l.id,'collect',diff,'cash','Thu thêm cọc (sửa hợp đồng)');
    l.depositPaid=leaseDepositCollected(l.id);
  }
  else{
    const sourceReservation=pendingLeadId?reservationForAppointment(pendingLeadId):null;
    commitOccupantDraft(occupant);
    lease={id:uid('l'),...values,status:'draft',signedAt:'',moveInAt:'',moveOutAt:'',terminationReason:'',
      createdAt:new Date().toISOString(),depositDeduct:0,depositRefund:0,settlementNote:'',roomHistory:[],renewals:[],statusHistory:[],documentFiles:[]};
    pushLeaseStatus(lease,'create',today(),'Tạo hợp đồng nháp');
    data.leases.push(lease);
    ...
    if(Number(lease.depositPaid)>0&&!sourceReservation)addDepositEntry(lease.id,'collect',Number(lease.depositPaid),'cash','Đặt cọc giữ chỗ');
  }
  reconcileRoomStatus(roomId);
  if(!id)completeLeadConversion(lease);  // lead → hợp đồng nháp, chỉ thành "Ký hợp đồng" sau khi xác nhận ký
...
window.signLease=function(id){
  if(!requireCan('edit','leases'))return;
  const l=getLease(id);if(!l)return;
  if(l.status!=='draft'){showToast('Chỉ ghi nhận ký cho hợp đồng nháp');return}
  l.signedAt=today();
  pushLeaseStatus(l,'sign',l.signedAt,'Ký hợp đồng');
  const leadIds=new Set(data.reservations.filter(r=>r.leaseId===l.id&&r.status==='converted').map(r=>r.appointmentId).filter(Boolean));
  data.appointments.filter(a=>a.convertedLeaseId===l.id||leadIds.has(a.id)).forEach(a=>{
    if(a.status!=='converted')leadLog(a,'Đã ký hợp đồng '+l.id,'convert');
    a.status='converted';a.convertedLeaseId=l.id;
  });
  saveData();renderAdmin();
  showToast('Đã ghi nhận ký hợp đồng ngày '+l.signedAt);
}
window.cancelLease=async function(id){
  if(!requireCan('edit','leases'))return;
  const l=getLease(id);if(!l||l.status!=='draft')return;
  if(Sync.isOn()&&!Sync.isAdmin()){showToast('Thiết bị chưa đăng nhập quyền quản lý');return}
  const held=leaseDepositHeld(id);
  if(held>0){showToast('Còn '+money(held)+' cọc đang giữ. Mở chi tiết hợp đồng → Hoàn cọc/Trừ cọc trước khi hủy.');openLeaseDetail(id);return}
  if(!confirm('Hủy hợp đồng nháp này? Phòng sẽ được tính lại trạng thái ngay.'))return;
  try{
    if(Sync.isOn()&&Sync.isAdmin()){
      await Sync.flush();
      const out=await Sync.leaseTransition({operation:'cancel',leaseId:id,date:today(),reason:'Hủy hợp đồng nháp'});
      Sync.applyActionResult(out);
    }else{
      l.status='cancelled';l.terminationReason='Hủy hợp đồng nháp';pushLeaseStatus(l,'cancel',today(),l.terminationReason);
      reconcileRoomStatus(l.roomId);saveData();
    }
    renderAdmin();renderPublic();showToast('Đã hủy hợp đồng nháp');
  }catch(err){showToast('Chưa hủy được hợp đồng: '+(err.message||err))}
}
window.deleteLease=function(id){
  ...
  if(depositEntries(id).length){showToast('Hợp đồng đã có giao dịch cọc nên không thể xóa. Hãy hủy để giữ lịch sử đối chiếu.');return}
  if((l.documentFiles||[]).length){showToast('Hợp đồng còn hồ sơ đính kèm. Hãy xóa hồ sơ trước hoặc hủy hợp đồng để giữ lịch sử.');return}
...
window.openCheckin=function(leaseId){
  if(!requireCan('edit','leases'))return;
  const l=getLease(leaseId);if(!l)return;
  if(l.status!=='draft'){showToast('Hợp đồng không còn ở trạng thái chờ nhận phòng');return}
  if(l.startDate&&l.startDate>today()){showToast('Hợp đồng bắt đầu ngày '+l.startDate+'. Chỉ nhận phòng vào đúng ngày hiệu lực hoặc điều chỉnh ngày bắt đầu trước.');return}
  const conflict=data.leases.find(x=>x.id!==l.id&&x.roomId===l.roomId&&liveLease(x));
  if(conflict){showToast('Phòng đang có hợp đồng hiệu lực khác. Hãy kiểm tra lại trước khi nhận phòng.');return}
  document.getElementById('checkinLeaseId').value=l.id;
  document.getElementById('checkinDate').value=l.startDate&&l.startDate>today()?l.startDate:today();
  document.getElementById('checkinDate').min=l.startDate||'';
  document.getElementById('checkinDate').max=today();
  const r=getRoom(l.roomId),t=getTenant(l.primaryTenantId);
  document.getElementById('checkinSummary').innerHTML=`<h4>${esc(r?.name||'')} · ${esc(t?.name||'')}</h4><p>Giá HĐ ${money(l.rentAmount)}/tháng${Number(l.monthlyDiscount)?` (giảm ${money(l.monthlyDiscount)}/tháng)`:''} · Cọc đang giữ ${money(leaseDepositHeld(l.id))}/${money(depositDueOf(l))}</p>`;
...
document.getElementById('checkinForm').addEventListener('submit',async e=>{
  ...
  if(l.status!=='draft'){showToast('Hợp đồng đã được xử lý trên thiết bị khác');return}
  if(l.startDate&&dateIn<l.startDate){showToast('Ngày nhận phòng không được trước ngày bắt đầu hợp đồng');return}
  const current=leaseOccupantsOf(l.id),primaryLinks=current.filter(x=>x.role==='primary'&&x.occupantId===l.primaryTenantId);
  if(primaryLinks.length!==1){showToast('Hợp đồng phải có đúng một người đại diện thanh toán');return}
  const room=getRoom(l.roomId),cap=Number(room?.capacity||0);
  if(cap&&current.length>cap){showToast(`Phòng chỉ có sức chứa ${cap} người, hiện hợp đồng có ${current.length} người.`);return}
  const handover=collectHandover(document.getElementById('checkinHandover'),l.id,'checkin');
  const btn=e.submitter;setBtnBusy(btn,true,'Đang nhận phòng…');
  try{
    if(Sync.isOn()&&Sync.isAdmin()){
      await Sync.flush();
      const out=await Sync.leaseTransition({operation:'checkin',leaseId:l.id,date:dateIn,handover});
      Sync.applyActionResult(out);
    }else{
      l.status='active';l.moveInAt=dateIn;if(!l.signedAt)l.signedAt=dateIn;pushLeaseStatus(l,'checkin',dateIn,'Nhận phòng '+(room?.name||''));
      data.handoverItems.push(...handover);
      current.forEach(lo=>{if(!lo.joinedAt)lo.joinedAt=dateIn;const t=getTenant(lo.occupantId);if(t){t.roomId=l.roomId;t.active=true;t.moveOutDate='';if(!t.moveInDate)t.moveInDate=dateIn}});
      const p0=getTenant(l.primaryTenantId);if(p0){p0.depositRequired=l.depositRequired;p0.depositPaid=leaseDepositCollected(l.id)}
      reconcileRoomStatus(l.roomId);saveData();
    }
    closeModal('checkinModal');renderAdmin();renderPublic();showToast('Đã nhận phòng, hợp đồng có hiệu lực từ '+dateIn);
    const primary=getTenant(l.primaryTenantId),acc=primary?accountForOccupant(primary.id):null;
    if(primary&&!(acc&&(acc.pin||acc.hasPin))&&!tenantHasPin(primary)&&confirm('Người đại diện chưa có mã PIN đăng nhập cư dân. Tạo PIN ngay bây giờ?'))resetTenantPin(primary.id);
  }catch(err){showToast('Chưa nhận phòng được: '+(err.message||err))}
  finally{setBtnBusy(btn,false)}
});
...
document.getElementById('occupantForm').addEventListener('submit',e=>{
  ...
  const cap=Number(getRoom(l.roomId)?.capacity||0),currentCount=leaseOccupantsOf(l.id).length;
  if(cap&&currentCount>=cap){showToast(`Phòng đã đủ sức chứa ${cap} người. Hãy cập nhật sức chứa hoặc chọn phòng khác.`);return}
  const otherLive=data.leaseOccupants.find(x=>x.occupantId===t.id&&!x.leftAt&&x.leaseId!==l.id&&liveLease(getLease(x.leaseId)));
  if(otherLive){showToast('Người này đang ở trong một hợp đồng hiệu lực khác');return}
  commitOccupantDraft(t);
  ...
  pushLeaseStatus(l,'occupant-add',document.getElementById('occupantJoinDate').value||today(),'Thêm người ở: '+t.name);
...
const LEASE_ACTIONS={
  ...
  deposit:(id,type)=>openDepositForm(id,type)
};
window.leaseAction=function(action,id,extra){
  const fn=LEASE_ACTIONS[action];
  if(!fn)return;
  closeModal('leaseDetailModal');
  setTimeout(()=>fn(id,extra),0);
}
...
window.removeOccupant=function(loId){
  ...
  if(lo.role==='primary'&&l&&['draft','active','ending'].includes(l.status)){showToast('Đây là người đại diện thanh toán. Hãy đổi người đại diện, hủy hợp đồng nháp hoặc thanh lý trước.');return}
  if(!confirm(`${t?.name||'Người này'} rời khỏi hợp đồng từ hôm nay?`))return;
  lo.leftAt=today();
  pushLeaseStatus(l,'occupant-leave',lo.leftAt,'Người ở rời đi: '+(t?.name||''));
...
window.makePrimary=function(loId){
  ...
  if(t){t.depositRequired=l.depositRequired;t.depositPaid=leaseDepositCollected(l.id)}
  pushLeaseStatus(l,'primary',today(),'Đổi đại diện thanh toán: '+(t?.name||''));
...
window.openTransferForm=function(leaseId){
  if(!requireCan('edit','leases'))return;
  const l=getLease(leaseId);if(!l)return;
  if(!liveLease(l)){showToast('Chỉ chuyển phòng cho hợp đồng đang hiệu lực');return}
  document.getElementById('transferLeaseId').value=l.id;
  const options=visibleRooms().filter(r=>r.id!==l.roomId&&!r.archived&&r.status!=='maintenance'&&!activeLeaseForRoom(r.id)&&!draftLeaseForRoom(r.id)&&!activeReservationForRoom(r.id))
  ...
  document.getElementById('transferDate').min=l.moveInAt||l.startDate||'';
  document.getElementById('transferDate').max=today();
  ...
  document.getElementById('transferHandoverOut').innerHTML=handoverRowsHtml(l.roomId,'checkout',l.id);
  const firstTarget=document.getElementById('transferRoom').value;
  document.getElementById('transferHandoverIn').innerHTML=handoverRowsHtml(firstTarget,'checkin',l.id);
  openModal('transferModal');
}
document.getElementById('transferRoom').addEventListener('change',e=>{
  const l=getLease(document.getElementById('transferLeaseId').value);
  document.getElementById('transferHandoverIn').innerHTML=handoverRowsHtml(e.target.value,'checkin',l?.id);
});
document.getElementById('transferForm').addEventListener('submit',async e=>{
  ...
  if(!liveLease(l)){showToast('Hợp đồng đã được xử lý trên thiết bị khác');return}
  if(eff>today()){showToast('Chưa hỗ trợ chuyển phòng vào ngày tương lai. Hãy thực hiện đúng ngày hiệu lực.');return}
  if(newRoom.archived||newRoom.status==='maintenance'||activeLeaseForRoom(newRoomId)||draftLeaseForRoom(newRoomId)||activeReservationForRoom(newRoomId)){showToast('Phòng mới không còn trống để chuyển');return}
  const count=leaseOccupantsOf(l.id).length,cap=Number(newRoom.capacity||0);
  if(cap&&count>cap){showToast(`Phòng mới chỉ có sức chứa ${cap} người, hợp đồng hiện có ${count} người.`);return}
  const oldRoomId=l.roomId;
  const keepRent=document.getElementById('transferKeepRent').checked;
  const handoverOut=collectHandover(document.getElementById('transferHandoverOut'),l.id,'transfer-out');
  const handoverIn=collectHandover(document.getElementById('transferHandoverIn'),l.id,'transfer-in');
  const btn=e.submitter;setBtnBusy(btn,true,'Đang chuyển phòng…');
  try{
    if(Sync.isOn()&&Sync.isAdmin()){
      await Sync.flush();
      const out=await Sync.leaseTransition({operation:'transfer',leaseId:l.id,date:eff,newRoomId,keepRent,newRent:Number(newRoom.price||0),handoverOut,handoverIn});
      Sync.applyActionResult(out);
    }else{
      const lastFrom=l.roomHistory.length?l.roomHistory[l.roomHistory.length-1].to:(l.moveInAt||l.startDate||'');
      l.roomHistory.push({roomId:oldRoomId,from:lastFrom,to:eff});l.roomId=newRoomId;l.propertyId=newRoom.propertyId;
      if(!keepRent){l.renewals.push({type:'rent-change',at:eff,oldRent:l.rentAmount,newRent:Number(newRoom.price||0)});l.rentAmount=Number(newRoom.price||0)}
      pushLeaseStatus(l,'transfer',eff,(getRoom(oldRoomId)?.name||oldRoomId)+' → '+newRoom.name);
      data.handoverItems.push(...handoverOut,...handoverIn);
      leaseOccupantsOf(l.id).forEach(lo=>{const t=getTenant(lo.occupantId);if(t)t.roomId=newRoomId});
      reconcileRoomStatus(oldRoomId);reconcileRoomStatus(newRoomId);saveData();
    }
    closeModal('transferModal');renderAdmin();renderPublic();openLeaseDetail(l.id);
    showToast(`Đã chuyển phòng từ ${eff}. Trạng thái hai phòng và biên bản bàn giao đã cập nhật.`);
  }catch(err){showToast('Chưa chuyển phòng được: '+(err.message||err))}
  finally{setBtnBusy(btn,false)}
});
...
window.openRenewForm=function(leaseId){
  if(!requireCan('edit','leases'))return;
  const l=getLease(leaseId);if(!l)return;
  if(!liveLease(l)){showToast('Chỉ gia hạn hợp đồng đang hiệu lực');return}
...
document.getElementById('renewForm').addEventListener('submit',e=>{
  ...
  pushLeaseStatus(l,'renew',today(),'Gia hạn đến '+newEnd);
...
window.openEndLease=function(leaseId){
  if(!requireCan('edit','leases'))return;
  const l=getLease(leaseId);if(!l)return;
  if(!liveLease(l)){showToast('Hợp đồng không còn hiệu lực để thanh lý');return}
  document.getElementById('endLeaseId').value=l.id;
  document.getElementById('endDate2').value=today();
  document.getElementById('endDate2').min=l.moveInAt||l.startDate||'';
  document.getElementById('endDate2').max=today();
  document.getElementById('endReason').value='';
  setMoneyInput('endDeduct',0);
  document.getElementById('endSettleNote').value='';
  document.getElementById('endRefundMethod').value='cash';
  document.getElementById('endRefundReference').value='';
  const r=getRoom(l.roomId),t=getTenant(l.primaryTenantId);
  document.getElementById('endSummary').innerHTML=`<h4>${esc(r?.name||'')} · ${esc(t?.name||'')}</h4><p>Cọc đang giữ theo sổ: <strong>${money(leaseDepositHeld(l.id))}</strong></p>`;
...
window.updateEndRefund=function(){
  ...
  if(Sync.isOn()&&!Sync.isAdmin()){showToast('Thiết bị chưa đăng nhập quyền quản lý');return}
  const held=leaseDepositHeld(l.id),deduct=Math.max(0,readMoneyInput('endDeduct'));
  const refund=Math.max(0,held-deduct);
  document.getElementById('endRefundPreview').innerHTML=
    `Cọc đang giữ ${money(held)} − trừ ${money(deduct)} = <strong>hoàn khách ${money(refund)}</strong>`;
}
document.getElementById('endLeaseForm').addEventListener('submit',async e=>{
  ...
  if(!liveLease(l)){showToast('Hợp đồng đã được xử lý trên thiết bị khác');return}
  if(l.moveInAt&&out<l.moveInAt){showToast('Ngày trả phòng không được trước ngày nhận phòng');return}
  const held=leaseDepositHeld(l.id),deduct=readMoneyInput('endDeduct');
  if(deduct<0||deduct>held){showToast('Số tiền trừ không được vượt quá cọc đang giữ '+money(held));return}
  const reason=document.getElementById('endReason').value.trim(),note=document.getElementById('endSettleNote').value.trim();
  if(!reason){showToast('Hãy nhập lý do kết thúc hợp đồng');return}
  if(deduct>0&&!note){showToast('Có trừ cọc nên cần ghi rõ nội dung đối chiếu');return}
  const refund=held-deduct,handover=collectHandover(document.getElementById('endHandover'),l.id,'checkout');
  if(!confirm(`Thanh lý hợp đồng, trừ ${money(deduct)} và hoàn ${money(refund)} cho khách?`))return;
  const btn=e.submitter;setBtnBusy(btn,true,'Đang thanh lý…');
  try{
    if(Sync.isOn()&&Sync.isAdmin()){
      await Sync.flush();
      const result=await Sync.leaseTransition({operation:'checkout',leaseId:l.id,date:out,reason,deduct,note,handover,
        refundMethod:document.getElementById('endRefundMethod').value,reference:document.getElementById('endRefundReference').value.trim()});
      Sync.applyActionResult(result);
    }else{
      l.status='ended';l.moveOutAt=out;l.terminationReason=reason;l.depositDeduct=deduct;l.depositRefund=refund;l.settlementNote=note;
      pushLeaseStatus(l,'checkout',out,`Trừ cọc ${money(deduct)} · hoàn ${money(refund)}`);
      if(deduct>0)addDepositEntry(l.id,'deduct',deduct,'',note||'Trừ cọc khi thanh lý',out);
      if(refund>0)addDepositEntry(l.id,'refund',refund,document.getElementById('endRefundMethod').value,'Hoàn cọc khi thanh lý',out);
      data.handoverItems.push(...handover);
      leaseOccupantsOf(l.id).forEach(lo=>{lo.leftAt=out;const t=getTenant(lo.occupantId);if(t&&!data.leaseOccupants.some(x=>x.occupantId===t.id&&!x.leftAt)){t.active=false;t.moveOutDate=out;t.roomId=''}});
      reconcileRoomStatus(l.roomId);saveData();
    }
    const finalLease=getLease(l.id);closeModal('endLeaseModal');renderAdmin();renderPublic();showToast(`Đã thanh lý hợp đồng. Hoàn cọc ${money(finalLease?.depositRefund??refund)} cho khách.`);
  }catch(err){showToast('Chưa thanh lý được: '+(err.message||err))}
  finally{setBtnBusy(btn,false)}
});
...
window.openRoomAssets=function(roomId){
  if(!requireCan('edit','rooms'))return;
...
function renderAssetRows(){
  ...
  document.getElementById('assetsRows').innerHTML=(list.length?`<table class="data-table"><thead><tr><th>Tên</th><th>SL</th><th>Tình trạng</th><th>Ghi chú</th><th></th></tr></thead><tbody>`+
    list.map(a=>`<tr>
      ...
      <td><input class="inline-input" type="number" min="1" value="${a.quantity}" data-evt="change" data-call="editAsset" data-a1="${a.id}" data-a2="quantity" data-a3="V"></td>
...
window.editAsset=function(id,field,value){
  if(!requireCan('edit','assets'))return;
  const a=data.assets.find(x=>x.id===id);if(!a)return;
  a[field]=field==='quantity'?Math.max(1,Number(value||1)):String(value);
  saveData();
}
window.addAssetRow=function(){
  if(!requireCan('create','assets'))return;
  const roomId=document.getElementById('assetsRoomId').value;
...
function leaseCardHtml(l){
  ...
  if(can('edit','leases')&&l.status==='draft')acts.push(`<button class="icon-btn" data-evt="click" data-call="openCheckin" data-a1="${l.id}">Nhận phòng</button>`,`<button class="icon-btn" data-evt="click" data-call="openLeaseForm" data-a1="${l.id}">Sửa</button>`,`<button class="icon-btn danger" data-evt="click" data-call="cancelLease" data-a1="${l.id}">Hủy</button>`);
  if(can('edit','leases')&&liveLease(l))acts.push(`<button class="icon-btn" data-evt="click" data-call="openRenewForm" data-a1="${l.id}">Gia hạn</button>`,`<button class="icon-btn" data-evt="click" data-call="openTransferForm" data-a1="${l.id}">Chuyển phòng</button>`,`<button class="icon-btn danger" data-evt="click" data-call="openEndLease" data-a1="${l.id}">Trả phòng</button>`);
  return `<div class="lease-card" data-lease="${l.id}">
    <div class="lease-card-main" data-evt="click" data-call="openLeaseDetail" data-a1="${l.id}">
      ...
      <div class="lease-sub">${icon('users',13)} ${esc(t?.name||'—')}${others?` +${others} người ở cùng`:''} · ${money(l.rentAmount)}/tháng</div>
      <div class="lease-sub">${esc(l.startDate||'?')} → ${esc(l.endDate||'không thời hạn')} · Cọc đang giữ ${money(leaseDepositHeld(l.id))}/${money(depositDueOf(l))}</div>
    </div>
    <div class="table-actions">${acts.join('')}</div>
  </div>`;
}

function fileSizeText(n){const x=Number(n||0);return x>=1048576?(x/1048576).toFixed(1)+' MB':Math.max(1,Math.round(x/1024))+' KB'}
window.pickLeaseDocuments=function(leaseId){
  if(!requireCan('edit','leases'))return;
  const l=getLease(leaseId);if(!l)return;
  if(!Sync.isOn()||!Sync.isAdmin()){showToast('Cần kết nối và đăng nhập quản lý để lưu hồ sơ hợp đồng');return}
  const input=document.createElement('input');input.type='file';input.accept='application/pdf,image/jpeg,image/png,image/webp,image/gif';input.multiple=true;
  input.onchange=()=>uploadLeaseDocuments(leaseId,[...(input.files||[])]);input.click();
}
async function uploadLeaseDocuments(leaseId,files){
  if(!getLease(leaseId)||!files.length)return;
  if(files.some(f=>f.size>6*1024*1024)){showToast('Mỗi hồ sơ phải nhỏ hơn 6MB');return}
  if(files.some(f=>!['application/pdf','image/jpeg','image/png','image/webp','image/gif'].includes(f.type))){showToast('Chỉ nhận PDF hoặc ảnh JPEG/PNG/WebP/GIF');return}
  try{
    showToast('Đang tải '+files.length+' hồ sơ…');
    await Sync.flush(); // bảo đảm hợp đồng đã tồn tại trên máy chủ trước khi gắn tệp riêng tư
    const current=getLease(leaseId);if(!current)throw new Error('Hợp đồng không còn tồn tại');
    for(const f of files){const saved=await Sync.uploadDocument(f,leaseId);current.documentFiles.push(saved)}
    saveData();await Sync.flush();openLeaseDetail(leaseId);showToast('Đã lưu hồ sơ hợp đồng ở chế độ riêng tư');
  }catch(err){showToast('Chưa tải được hồ sơ: '+(err.message||err))}
}
window.openLeaseDocument=async function(leaseId,fileId){
  const l=getLease(leaseId),meta=l?.documentFiles.find(x=>x.id===fileId);if(!meta)return;
  try{
    showToast('Đang mở hồ sơ…');
    const out=await Sync.fetchPrivateFile(fileId,leaseId),bin=atob(out.data),bytes=new Uint8Array(bin.length);
    for(let i=0;i<bin.length;i++)bytes[i]=bin.charCodeAt(i);
    const url=URL.createObjectURL(new Blob([bytes],{type:out.mime||meta.mime||'application/octet-stream'}));
    const a=document.createElement('a');a.href=url;a.target='_blank';a.rel='noopener';a.download=out.name||meta.name||'ho-so';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),60000);
  }catch(err){showToast('Không mở được hồ sơ: '+(err.message||err))}
}
window.deleteLeaseDocument=async function(leaseId,fileId){
  if(!requireCan('edit','leases'))return;
  const l=getLease(leaseId),meta=l?.documentFiles.find(x=>x.id===fileId);if(!meta)return;
  if(!confirm('Xóa hồ sơ “'+meta.name+'”? Tệp sẽ được chuyển vào thùng rác Drive.'))return;
  try{await Sync.deletePrivateFile(fileId,leaseId);l.documentFiles=l.documentFiles.filter(x=>x.id!==fileId);saveData();openLeaseDetail(leaseId);showToast('Đã xóa hồ sơ hợp đồng')}
  catch(err){showToast('Chưa xóa được hồ sơ: '+(err.message||err))}
}
...
function renderLeases(){
  ...
  if(st.sort==='end')list.sort((a,b)=>(a.endDate||'9999-12-31').localeCompare(b.endDate||'9999-12-31'));
  else if(st.sort==='room')list.sort((a,b)=>String(getRoom(a.roomId)?.name||'').localeCompare(String(getRoom(b.roomId)?.name||''),'vi'));
  else if(st.sort==='tenant')list.sort((a,b)=>String(getTenant(a.primaryTenantId)?.name||'').localeCompare(String(getTenant(b.primaryTenantId)?.name||''),'vi'));
  else if(st.sort==='newest')list.sort((a,b)=>String(b.createdAt||'').localeCompare(a.createdAt||''));
  else list.sort((a,b)=>(order[a.status]??9)-(order[b.status]??9)||String(b.createdAt||'').localeCompare(a.createdAt||''));
  ...
  root.innerHTML=`<div class="panel"><div class="panel-head"><div><h3>Hợp đồng thuê</h3><p>Giữ chỗ → nháp → nhận phòng → đang hiệu lực → gia hạn/chuyển phòng → thanh lý</p></div>${can('create','leases')?`<button class="btn btn-primary" onclick="openLeaseForm()">${icon('plus',15)} Hợp đồng mới</button>`:''}</div>
    <div class="table-tools">
      ...
      <select aria-label="Sắp xếp hợp đồng" onchange="uiSet('leases','sort',this.value)">
        <option value="" ${!st.sort?'selected':''}>Ưu tiên cần xử lý</option>
        <option value="end" ${st.sort==='end'?'selected':''}>Sắp hết hạn trước</option>
        <option value="newest" ${st.sort==='newest'?'selected':''}>Tạo mới nhất</option>
        <option value="room" ${st.sort==='room'?'selected':''}>Theo tên phòng</option>
        <option value="tenant" ${st.sort==='tenant'?'selected':''}>Theo người đại diện</option>
      </select>
    </div>
    ${alertHtml}
...
function timelineEvents(l){
  ...
  push(l.createdAt,'filetext','Tạo hợp đồng nháp','');
  if(leaseDepositCollected(l.id)>0)push(l.signedAt||l.createdAt,'receipt','Đặt cọc giữ chỗ',money(leaseDepositCollected(l.id))+' / đang giữ '+money(leaseDepositHeld(l.id)));
  push(l.signedAt,'check','Ký hợp đồng','');
  const hoIn=data.handoverItems.filter(h=>h.leaseId===l.id&&h.phase==='checkin');
  push(l.moveInAt,'key','Nhận phòng & bàn giao',hoIn.length?hoIn.length+' hạng mục tài sản đầu vào':'');
  leaseOccupantsOf(l.id,false).forEach(lo=>{
    const t=getTenant(lo.occupantId);
    if(lo.joinedAt&&lo.joinedAt!==(l.moveInAt||l.startDate))push(lo.joinedAt,'plus','Thêm người ở: '+(t?.name||''),lo.role==='primary'?'Đại diện thanh toán':'Ở cùng');
    if(lo.leftAt&&lo.leftAt!==l.moveOutAt)push(lo.leftAt,'logout','Rời đi: '+(t?.name||''),'');
  });
  l.roomHistory.forEach(h=>{const r=getRoom(h.roomId);push(h.to,'home','Chuyển khỏi phòng '+(r?.name||h.roomId),'Ở từ '+(h.from||'?')+' đến '+h.to)});
  l.renewals.forEach(rn=>{
    if(rn.type==='renew')push(rn.at,'clock','Gia hạn đến '+rn.newEnd,rn.oldRent!==rn.newRent?money(rn.oldRent)+' → '+money(rn.newRent)+'/tháng':'');
    else push(rn.at,'receipt','Điều chỉnh giá',money(rn.oldRent)+' → '+money(rn.newRent)+'/tháng');
  });
  (l.statusHistory||[]).filter(h=>['primary','cancel'].includes(h.type)).forEach(h=>{
    push(h.at,h.type==='primary'?'users':'x',h.type==='primary'?'Đổi đại diện thanh toán':'Hủy hợp đồng',`${h.detail||''}${h.by?' · '+h.by:''}`);
  });
  data.invoices.filter(i=>i.leaseId===l.id||i.tenantId===l.primaryTenantId).forEach(i=>{
    push(i.createdAt,'receipt','Hóa đơn tháng '+i.month,money(i.total)+' · '+({paid:'Đã thanh toán',partial:'Trả một phần',unpaid:'Chưa thanh toán'}[effectiveInvoiceStatus(i)]||''));
    (i.payments||[]).forEach(p2=>push(p2.date||p2.at,'receipt','Thu tiền HĐ '+i.month,money(p2.amount)+(p2.method?' · '+({cash:'Tiền mặt',bank:'Chuyển khoản',other:'Khác'}[p2.method]||p2.method):'')));
  });
  const hoOut=data.handoverItems.filter(h=>h.leaseId===l.id&&h.phase==='checkout');
  if(l.moveOutAt){
    push(l.moveOutAt,'logout','Trả phòng & bàn giao ra',hoOut.length?hoOut.length+' hạng mục kiểm tra đầu ra':'');
    push(l.moveOutAt,'check','Thanh lý hợp đồng',
...
window.openLeaseDetail=function(id){
  ...
  const occHtml=occ.map(lo=>{
    ...
    return `<div class="occ-row ${gone?'occ-gone':''}">
      ...
      ${can('edit','leases')&&!gone&&liveLease(l)?`<div class="table-actions">${lo.role!=='primary'?`<button class="icon-btn" data-evt="click" data-call="makePrimary" data-a1="${lo.id}">Làm đại diện</button><button class="icon-btn danger" data-evt="click" data-call="removeOccupant" data-a1="${lo.id}">Rời đi</button>`:''}</div>`:''}
    </div>`;
  }).join('');
  const acts=[];
  if(can('edit','leases')&&l.status==='draft')acts.push(`<button class="btn btn-primary" data-evt="click" data-call="leaseAction" data-a1="checkin" data-a2="${l.id}">${icon('key',16)} Nhận phòng</button>`,`<button class="btn btn-light" data-evt="click" data-call="signLeaseThenDetail" data-a1="${l.id}">${icon('check',16)} Ký HĐ</button>`,`<button class="btn btn-light" data-evt="click" data-call="leaseAction" data-a1="editDraft" data-a2="${l.id}">Sửa nháp</button>`);
  if(can('edit','leases')&&liveLease(l))acts.push(`<button class="btn btn-light" data-evt="click" data-call="leaseAction" data-a1="addOccupant" data-a2="${l.id}">${icon('plus',16)} Thêm người ở</button>`,
    `<button class="btn btn-light" data-evt="click" data-call="leaseAction" data-a1="renew" data-a2="${l.id}">${icon('clock',16)} Gia hạn</button>`,
    `<button class="btn btn-light" data-evt="click" data-call="leaseAction" data-a1="transfer" data-a2="${l.id}">${icon('home',16)} Chuyển phòng</button>`,
    `<button class="btn btn-danger" data-evt="click" data-call="leaseAction" data-a1="endLease" data-a2="${l.id}">${icon('logout',16)} Trả phòng / Thanh lý</button>`);
  ...
  document.getElementById('leaseDetailBody').innerHTML=`
    <div class="lease-head-grid">
      ...
      <div class="kv"><span>Cọc đang giữ (theo sổ)</span><strong>${money(leaseDepositHeld(l.id))} / ${money(depositDueOf(l))}${Number(l.depositDiscount)?` <small class="disc-tag">đã giảm ${money(l.depositDiscount)}</small>`:''}</strong></div>
    ...
    ${l.note?`<p class="muted-text">${icon('filetext',14)} ${esc(l.note)}</p>`:''}
    ${l.settlementNote?`<p class="muted-text">${icon('check',14)} ${esc(l.settlementNote)}</p>`:''}
    <h4 class="lease-section-title">Hồ sơ hợp đồng (${l.documentFiles.length})</h4>
    ${l.documentFiles.map(f=>`<div class="occ-row"><div><strong>${icon(f.mime==='application/pdf'?'filetext':'camera',14)} ${esc(f.name)}</strong><br><span style="color:var(--muted)">${fileSizeText(f.size)} · tải lên ${esc(String(f.uploadedAt||'').slice(0,10))}</span></div>${['owner','manager'].includes(currentRole())?`<div class="table-actions"><button class="icon-btn" data-evt="click" data-call="openLeaseDocument" data-a1="${l.id}" data-a2="${esc(f.id)}">Mở / tải</button>${can('edit','leases')&&!['ended','cancelled'].includes(l.status)?`<button class="icon-btn danger" data-evt="click" data-call="deleteLeaseDocument" data-a1="${l.id}" data-a2="${esc(f.id)}">Xóa</button>`:''}</div>`:'<span class="badge badge-unpaid">Hạn chế theo vai trò</span>'}</div>`).join('')||'<div class="empty">Chưa có PDF/ảnh hợp đồng, CCCD hoặc biên bản.</div>'}
    ${can('edit','leases')&&!['ended','cancelled'].includes(l.status)?`<div class="table-actions" style="margin-top:6px"><button class="icon-btn" data-evt="click" data-call="pickLeaseDocuments" data-a1="${l.id}">+ Tải PDF / ảnh hồ sơ</button></div>`:''}
    ...
    ${leaseServicesOf(l.id,monthNow()).map(ls=>{const sv=getService(ls.serviceId);if(!sv)return'';
      ...
      return `<div class="occ-row"><div><strong>${esc(sv.name)}</strong> · ${calcTypeLabel(sv.calcType)} · ${money(price)}${sv.unit?'/'+esc(sv.unit):''}${sv.calcType==='perUnit'?` × ${ls.quantity}`:''}${ls.discountPercent?` · giảm ${ls.discountPercent}%`:''}<br><span style="color:var(--muted)">Từ ${esc(ls.effectiveFrom||'?')}</span></div>${liveLease(l)&&can('edit','leaseServices')?`<div class="table-actions"><button class="icon-btn danger" data-evt="click" data-call="endLeaseService" data-a1="${ls.id}">Ngừng</button></div>`:''}</div>`}).join('')||'<div class="empty">Chưa gắn dịch vụ nào.</div>'}
    ${liveLease(l)&&can('create','leaseServices')?`<div class="table-actions" style="margin-top:6px"><button class="icon-btn" data-evt="click" data-call="leaseAction" data-a1="addService" data-a2="${l.id}">+ Gắn dịch vụ</button></div>`:''}
    ...
    ${depositEntries(l.id).map(x=>`<div class="rem-log">${icon(x.type==='collect'?'receipt':x.type==='refund'?'back':'alert',14)} ${({collect:'Thu cọc',refund:'Hoàn cọc',deduct:'Trừ cọc'})[x.type]} ${money(x.amount)} · ${esc(x.at)}${x.note?` · ${esc(x.note)}`:''}</div>`).join('')||'<div class="empty">Chưa có giao dịch cọc.</div>'}
    ${can('edit','depositLedger')&&!['ended','cancelled'].includes(l.status)?`<div class="table-actions" style="margin-top:6px"><button class="icon-btn" data-evt="click" data-call="leaseAction" data-a1="deposit" data-a2="${l.id}" data-a3="collect">+ Thu cọc</button><button class="icon-btn" data-evt="click" data-call="leaseAction" data-a1="deposit" data-a2="${l.id}" data-a3="refund">Hoàn cọc</button></div>`:''}
    <h4 class="lease-section-title">Dòng thời gian</h4>
    <div class="timeline">${timelineEvents(l).map(e2=>`
      <div class="tl-item"><div class="tl-dot">${icon(e2.icon,13)}</div>
...
function renderSettings(){
  ...
  const editSettings=can('edit','settings'),isOwner=ownerOnly();
  const roleText=Sync.isAdmin()?`${ROLE_LABEL[currentRole()]||'Quản lý'} · ${editSettings?'được sửa cài đặt':'chỉ xem cài đặt'}`:connected?'Chỉ xem':'Chưa kết nối';
  ...
  const deviceBtns=(connected&&Sync.isAdmin())?`
      ...
      ${isOwner?'<button class="btn btn-danger" onclick="logoutAllDevices()">Đăng xuất tất cả thiết bị</button>':''}`:'';
  ...
  <div class="conn-card" style="grid-column:1/-1">
    ...
    ${isOwner?`<label>Đổi mật khẩu quản lý</label>
  ...
  <div class="settings-card"><h3>Thương hiệu & thông tin quản lý</h3>
    ...
    <label style="display:block;font-size:12.5px;color:var(--muted);margin:10px 0 4px">Ngày thu hàng tháng mặc định (1–28)</label>
  ...
  if(!editSettings)document.querySelectorAll('#view-settings [id^="set"]').forEach(el=>{el.disabled=true;el.setAttribute('aria-disabled','true')});
}
...
function auditCardHtml(){
  ...
  <div class="table-tools"><span class="tt-search">${icon('search',15)}<input value="${esc(st.q||'')}" placeholder="Lọc theo người, hành động, bảng…" aria-label="Lọc nhật ký" oninput="uiSet('audit','q',this.value)"></span></div>
...
function purgeAdminData(){
  const SENSITIVE=['tenants','invoices','leases','leaseOccupants','accounts','payments','depositLedger','reservations',
...
function validateImport(x){
  ...
  const cols=['properties','rooms','tenants','utilityReadings','invoices','appointments','reservations'];
  const counts=[];
  for(const c of cols){
    const arr=x[c];
    ...
    counts.push(arr.length+' '+({properties:'căn trọ',rooms:'phòng',tenants:'người thuê',utilityReadings:'chỉ số điện nước',invoices:'hóa đơn',appointments:'lịch hẹn',reservations:'phiếu giữ chỗ'})[c]);
  }
...
window.resetDemo=function(){
  ...
  if(Sync.isOn()){showToast('Không thể nạp dữ liệu mẫu khi đã kết nối máy chủ. Hãy dùng một trình duyệt thử nghiệm không kết nối.');return}
...
document.addEventListener('click',e=>{
  ...
  if(close){e.preventDefault();requestCloseModal(close.dataset.closeModal);return}
  const modal=e.target.classList?.contains('modal')?e.target:null;
  if(modal)requestCloseModal(modal.id);
});
function markModalDirty(e){
  if(!e.isTrusted)return;
  const form=e.target.closest?.('.modal:not(.hidden) form');
  const modal=form?.closest('.modal');
  if(modal)modal.dataset.dirty='1';
}
document.addEventListener('input',markModalDirty,true);
document.addEventListener('change',markModalDirty,true);
document.addEventListener('drop',markModalDirty,true);
document.addEventListener('click',e=>{
  if(!e.isTrusted)return;
  const button=e.target.closest?.('.modal:not(.hidden) form button');
  if(button&&button.type!=='submit'&&!button.hasAttribute('data-close-modal'))button.closest('.modal').dataset.dirty='1';
},true);
...
/** v4.2.10 — Khi khung giờ bị trùng: tự nhảy sang khung giờ trống gần nhất còn lại. */
function suggestOtherSlot(payload){
  const sel=document.getElementById('appointmentTime');
  if(!sel)return;
  const taken=new Set(data.appointments
    .filter(a=>a.roomId===payload.roomId&&a.date===payload.date&&!['cancelled','lost'].includes(a.status))
    .map(a=>a.time));
  taken.add(payload.time);
  const free=[...sel.options].filter(o=>!o.disabled).map(o=>o.value).filter(v=>v&&!taken.has(v));
  if(free.length){
    sel.value=free[0];
    setTimeout(()=>showToast(`Khung ${payload.time} đã có người. Em đã chuyển sang ${free[0]} — anh/chị bấm Gửi lại giúp em nhé.`),2200);
  }else{
    setTimeout(()=>showToast('Ngày này đã kín lịch. Anh/chị chọn ngày khác giúp em nhé.'),2200);
  }
}
...
document.getElementById('bookingForm').addEventListener('submit',async e=>{e.preventDefault();
  ...
  if(Sync.isOn()){
    ...
    try{
      ...
      // v4.2.10 — Phân biệt hai loại thất bại:
      //  (1) máy chủ SỐNG nhưng từ chối có lý do (trùng giờ, quá số lần, ngoài khung giờ…)
      //      → nói đúng lý do đó, KHÔNG bày lối liên hệ dự phòng vì không phải hỏng hệ thống.
      //  (2) hỏng đường truyền / sai cấu hình → câu chung + mở lối gọi, Zalo.
      const systemFail=!err.serverAnswered||['gateway','unknownAction','auth'].includes(err.code||'');
      if(systemFail){
        showToast('Chưa gửi được lịch hẹn lên hệ thống. Anh/chị gọi hoặc nhắn Zalo giúp em nhé.');
        showBookingFallback(payload,err);
      }else{
        showToast(err.message||'Chưa gửi được lịch hẹn.');
        document.getElementById('bookingFallback')?.classList.add('hidden');
        // Trùng giờ / hết lượt: gợi ý ngay khung giờ khác cho khách khỏi loay hoay
        if(/đã đặt đúng khung giờ|đã có người hẹn|quá nhiều yêu cầu/i.test(err.message||'')){
          await refreshBookingSlots();
          suggestOtherSlot(payload);
        }
      }
    }finally{setBtnBusy(btn,false)}
    return;
...
document.getElementById('roomForm').addEventListener('submit',async e=>{e.preventDefault();const id=document.getElementById('roomId').value;if(!requireCan(id?'edit':'create','rooms'))return;const btn=document.getElementById('roomSubmit');setBtnBusy(btn,true,'Đang lưu…');let imageIds;try{imageIds=await commitImageState(roomImageState,'room',id||'')}catch(err){setBtnBusy(btn,false);showToast('Chưa lưu được ảnh phòng: '+(err.message||err));return}setBtnBusy(btn,false);let selectedStatus=document.getElementById('roomStatus').value;if(selectedStatus==='reserved'&&!activeReservationForRoom(id)&&!(draftLeaseForRoom(id)&&leaseDepositHeld(draftLeaseForRoom(id).id)>0))selectedStatus='available';const item={id:id||uid('r'),propertyId:document.getElementById('roomProperty').value,name:document.getElementById('roomName').value.trim(),price:readMoneyInput('roomPrice'),deposit:readMoneyInput('roomDeposit'),area:Number(document.getElementById('roomArea').value||0),capacity:Number(document.getElementById('roomCapacity').value||1),type:document.getElementById('roomType').value,status:selectedStatus,electricRate:Number(document.getElementById('roomElectricRate').value||0),waterMode:document.getElementById('roomWaterMode').value,waterRate:Number(document.getElementById('roomWaterRate').value||0),waterFixed:Number(document.getElementById('roomWaterFixed').value||0),amenities:document.getElementById('roomAmenities').value.split(',').map(x=>x.trim()).filter(Boolean),note:document.getElementById('roomNote').value.trim(),imageIds,
...
{let slug=slugifyVN(document.getElementById('roomSlug').value)||slugifyVN((getProperty(item.propertyId)?.name||'')+' '+item.name)||('phong-'+item.id);
 ...
 item.slug=slug}
if(id)Object.assign(getRoom(id),item);else data.rooms.push(item);reconcileRoomStatus(item.id);const effective=getRoom(item.id).status;saveData();closeModal('roomModal');renderAdmin();renderPublic();const adjusted=effective!==selectedStatus?` Trạng thái được hệ thống giữ ở “${statusLabel(effective)}” theo hợp đồng/giữ chỗ hiện tại.`:'';showToast((id?'Đã cập nhật phòng và ảnh':'Đã thêm phòng')+adjusted)});
...
document.getElementById('tenantForm').addEventListener('submit',async e=>{
  ...
  if(!requireCan(editingId?'edit':'create','tenants'))return;
  ...
  const lease0=id?leaseForOccupant(item.id):null;
  if(active&&!lease0){showToast('Để kích hoạt người ở, hãy tạo hợp đồng nháp rồi dùng “Nhận phòng”. Hồ sơ mới nên để trạng thái Chưa ở.');return}
  if(!active&&!lease0){item.roomId='';item.moveInDate='';item.depositRequired=0;item.depositPaid=0}
  if(lease0&&liveLease(lease0)&&lease0.primaryTenantId===item.id){
    if(!active){showToast('Không thể trả phòng từ hồ sơ người ở. Hãy mở Hợp đồng → Trả phòng / Thanh lý.');return}
    if(roomId!==lease0.roomId){showToast('Không thể chuyển phòng từ hồ sơ người ở. Hãy mở Hợp đồng → Chuyển phòng.');return}
    item.roomId=lease0.roomId;item.active=true;item.moveOutDate='';
    item.depositRequired=lease0.depositRequired;item.depositPaid=leaseDepositCollected(lease0.id);
  }
  if(id)Object.assign(getTenant(id),item);else data.tenants.push(item);
  // P1: phòng/trạng thái/cọc của hợp đồng chỉ thay đổi ở đúng nghiệp vụ Hợp đồng.
...
document.getElementById('invoiceForm').addEventListener('submit',e=>{e.preventDefault();
  if(!requireCan('create','invoices'))return;
...
document.getElementById('loginForm').addEventListener('submit',async e=>{
  ...
  if(value===ADMIN_PASSWORD){localStorage.setItem('huyrooms_admin_v3','true');field.value='';closeModal('loginModal');showAdmin()}
...
window.clearResidentDevice=function(){
  ...
  residentValidated=false;saveResidentSession(null);currentResidentId=null;
  showPublic();showToast('Đã xóa dữ liệu cư dân trên thiết bị này');
}
document.getElementById('residentLogout').addEventListener('click',async()=>{
  // Thu hồi đúng phiên máy này trên máy chủ, sau đó mới xóa bản lưu cục bộ.
  const sess=residentSession;
  if(sess&&Sync.isOn()&&sess.sessionKey){try{await Sync.residentLogout(sess.phone,sess.sessionKey)}catch(err){showToast('Đã xóa dữ liệu trên máy; máy chủ chưa xác nhận đăng xuất do mất kết nối.')}}
  residentValidated=false;saveResidentSession(null);currentResidentId=null;showPublic();
});document.getElementById('backPublic').addEventListener('click',showPublic);
document.getElementById('quickAddTenant').addEventListener('click',()=>openTenantForm());document.getElementById('quickAddUtility').addEventListener('click',()=>openUtilityForm());document.getElementById('quickAddInvoice').addEventListener('click',()=>openInvoiceForm());
...
/* ==================================================================
   ...
   ================================================================== */
const CALL_WHITELIST=new Set(["archiveService", "attachMeterPhoto", "bulkAdjust", "cancelHold", "cancelLease", "closeModal", "copyBookingMsg", "copyInvoiceText", "zaloSendNotice", "zaloToTenant", "runConnectionCheck", "deleteAsset", "deleteLeaseDocument", "deleteNotice", "deleteProperty", "deleteReading", "deleteRoom", "doReverse", "editAsset", "endLeaseService", "gallerySelect", "galleryStep", "leaseAction", "makePrimary", "meterInput", "openBooking", "openCheckin", "openEndLease", "openGallery", "openInvoiceForm", "openInvoicePdf", "openLeaseDetail", "openLeaseDocument", "openHoldForm", "openLeaseForm", "openMeterPhotos", "openPropertyForm", "openReceiptPdf", "openReminder", "openRenewForm", "openResidentInvoice", "openResidentMeterPhotos", "openRoomAssets", "openRoomForm", "openServiceForm", "openStaffForm", "openTicketDetail", "openTransferForm", "openUtilityForm", "palRun", "pickLeaseDocuments", "quickRoomMoney", "quickRoomStatus", "recordPayment", "removeOccupant", "resetStaffPass", "restoreAutoBackup", "restoreProperty", "restoreRoom", "rsRemoveFile", "setResidentTab", "signLease", "signLeaseThenDetail", "ticketAction", "togglePropertyRooms", "toggleAdvancedFilters", "toggleRoomSearch", "toggleAmenityFilter", "toggleStaff", "uiSet", "unlockReading"]);
...
document.addEventListener('keydown',e=>{
  const visible=visibleModals().pop();
  if(!visible)return;
  if(e.key==='Escape'){e.preventDefault();requestCloseModal(visible.id);return}
  if(visible.id==='galleryModal'&&e.key==='ArrowLeft'){e.preventDefault();galleryStep(-1)}
  if(visible.id==='galleryModal'&&e.key==='ArrowRight'){e.preventDefault();galleryStep(1)}
  if(e.key==='Tab'){
    const items=modalFocusable(visible);if(!items.length){e.preventDefault();visible.focus?.();return}
...
window.addEventListener('server-conflict',e=>{
  ...
  showToast(`Cảnh báo: máy chủ TỪ CHỐI ghi đè ${list}${(e.detail||[]).length>3?'…':''} — bản ghi đã được thiết bị khác sửa trước. Bản máy chủ được giữ; xem lại rồi sửa tiếp nếu cần.`);
...
window.addEventListener('sync-conflict',e=>{
  ...
  showToast(`Cảnh báo xung đột dữ liệu tài chính: ${list}${more} vừa được sửa trên thiết bị khác trong lúc máy này cũng sửa. Bản trên máy chủ được giữ; mở Nhật ký để đối chiếu trước khi thao tác tiếp.`);
...
Sync.attach({
  ...
  rerender:()=>{
    ...
    const publicVisible=!document.getElementById('publicApp')?.classList.contains('hidden');
    const residentVisible=!document.getElementById('residentApp')?.classList.contains('hidden');
    if(adminVisible)renderAdmin();
    if(publicVisible)renderPublic();
    if(residentVisible&&currentResidentId&&!residentSession)renderResident();
  },
  toast:showToast
});
...
if(Sync.isOn()&&!Sync.isAdmin()&&!(Sync.state.since>0)&&looksLikeDemo()){
  ['properties','rooms','tenants','utilityReadings','invoices','appointments','reservations'].forEach(c=>{data[c]=[]});
  Sync.baseline={};
}
saveLocal();
updateLoginHint();
