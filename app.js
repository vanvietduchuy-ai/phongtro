...
function renderTenants(){const root=document.getElementById('view-tenants');
  ...
  <div class="table-wrap"><table class="data-table"><thead><tr><th>Người thuê</th><th>Điện thoại</th><th>Phòng</th><th>Cọc phải đóng</th><th>Đã nhận</th><th>Còn thiếu</th><th>Tài khoản</th><th></th></tr></thead><tbody>${slice.length?slice.map(t=>{const r=getRoom(t.roomId),p=getProperty(r?.propertyId);const hr=tenantHolding(t)||tenantHoldExpired(t)?getRoom(t.holdRoomId):null;
    return `<tr class="${tenantHoldExpired(t)?'row-hold-expired':''}"><td><strong>${esc(t.name)}</strong><br>${tenantStatusBadge(t)}${hr?`<br><span style="color:var(--muted)">Giữ ${esc(hr.name)} · ${esc(t.holdFrom||'')}→${esc(t.holdUntil||'')} · ${money(t.holdAmount)}</span>`:''}</td><td>${esc(t.phone)}</td><td>${esc(p?.name||(hr?getProperty(hr.propertyId)?.name:'')||'')} · ${esc(r?.name||hr?.name||'-')}</td><td>${money(t.depositRequired)}</td><td>${money(t.depositPaid)}</td><td>${money(Math.max(0,t.depositRequired-t.depositPaid))}</td><td>${tenantHasPin(t)?'<span class="badge badge-paid">Đã có PIN</span>':'<span class="badge badge-unpaid">Chưa có PIN</span>'}</td><td><div class="table-actions">${t.phone?`<button class="icon-btn btn-zalo" data-evt="click" data-call="zaloToTenant" data-a1="${esc(t.id)}">${icon('message',13)} Zalo</button>`:''}${canManageHold()?(tenantHolding(t)||tenantHoldExpired(t)?`<button class="icon-btn" data-evt="click" data-call="cancelHold" data-a1="${esc(t.id)}">Hủy giữ chỗ</button>`:`<button class="icon-btn" data-evt="click" data-call="openHoldForm" data-a1="${esc(t.id)}">Giữ chỗ</button>`):''}<button class="icon-btn" data-act="tenantEdit" data-id="${esc(t.id)}">Sửa</button><button class="icon-btn" data-act="tenantPin" data-id="${esc(t.id)}">Đặt lại PIN</button><button class="icon-btn danger" data-act="tenantDelete" data-id="${esc(t.id)}">${data.invoices.some(i=>i.tenantId===t.id)?'Lưu trữ':'Xóa'}</button></div></td></tr>`}).join(''):`<tr><td colspan="8">${data.tenants.length?emptyState('search','Không khớp bộ lọc','Thử từ khóa khác hoặc chuyển bộ lọc về Tất cả.',''):emptyState('users','Chưa có người thuê','Tạo hợp đồng từ CRM khách xem phòng, hoặc thêm trực tiếp tại đây.',`<button class="btn btn-primary" onclick="openTenantForm()">${icon('plus',15)} Thêm người thuê</button>`)}</td></tr>`}</tbody></table></div>${nav}</div>`}
...
/* ==================================================================
   v4.3 — ĐẶT CỌC GIỮ CHỖ CHO NGƯỜI THUÊ (từ ngày → đến ngày)
   Phòng chuyển sang "Đã giữ chỗ"; người thuê ở trạng thái
   "Đang đặt cọc giữ chỗ" cho tới khi nhận phòng hoặc hết hạn giữ.
   ================================================================== */
/** Người thuê đang giữ chỗ hợp lệ (đã cọc, chưa quá hạn, chưa vào ở). */
function tenantHolding(t){
  if(!t||!t.holdRoomId||!t.holdUntil)return false;
  if(activeLeaseForTenant(t.id))return false;      // đã nhận phòng thì không còn là giữ chỗ
  return t.holdUntil>=today();
}
/** Giữ chỗ đã quá hạn mà chưa xử lý. */
function tenantHoldExpired(t){
  return !!(t&&t.holdRoomId&&t.holdUntil&&t.holdUntil<today()&&!activeLeaseForTenant(t.id));
}
...
function tenantStatus(t){
  ...
  if(tenantHolding(t))return {key:'holding',label:'Đang đặt cọc giữ chỗ',cls:'partial'};
  if(tenantHoldExpired(t))return {key:'hold_expired',label:'Giữ chỗ đã hết hạn',cls:'overdue'};
...
/** Phòng đang bị giữ chỗ bởi người thuê nào (nếu có). */
function holderOfRoom(roomId){
  return data.tenants.find(t=>t.holdRoomId===roomId&&tenantHolding(t));
}

/** Giữ chỗ gắn với TIỀN CỌC thật nên chỉ Chủ nhà / Quản lý được đặt và hủy.
 * Kế toán và Nhân viên không có quyền ghi bảng Người thuê ở máy chủ, nên nếu
 * cho bấm thì thao tác sẽ bị máy chủ bỏ qua — thà chặn ngay và nói rõ. */
...
window.openHoldForm=function(tenantId){
  ...
  const rooms=visibleRooms().filter(r=>!r.archived&&(r.status==='available'||r.id===t.holdRoomId));
  sel.innerHTML=rooms.map(r=>{const p=getProperty(r.propertyId);
    return `<option value="${r.id}" ${r.id===t.holdRoomId?'selected':''}>${esc(p?.name||'')} · ${esc(r.name)} — cọc niêm yết ${money(r.deposit)}</option>`}).join('')
    ||'<option value="">Không còn phòng trống</option>';
  document.getElementById('holdFrom').value=t.holdFrom||today();
  document.getElementById('holdFrom').min=today();
  document.getElementById('holdUntil').value=t.holdUntil||nextDayISO(7);
  document.getElementById('holdUntil').min=today();
  setMoneyInput('holdAmount',t.holdAmount||Number(getRoom(sel.value)?.deposit||0));
  document.getElementById('holdNote').value=t.holdNote||'';
  openModal('holdModal');
}
document.getElementById('holdForm')?.addEventListener('submit',e=>{
  ...
  const roomId=document.getElementById('holdRoom').value;
  const from=document.getElementById('holdFrom').value;
  const until=document.getElementById('holdUntil').value;
  const amount=Math.max(0,readMoneyInput('holdAmount'));
  if(!roomId){showToast('Chọn phòng cần giữ chỗ.');return}
  if(!from||!until||until<from){showToast('Ngày giữ chỗ chưa hợp lệ: ngày kết thúc phải sau ngày bắt đầu.');return}
  if(amount<=0){showToast('Nhập số tiền cọc giữ chỗ.');return}
  const other=holderOfRoom(roomId);
  if(other&&other.id!==t.id){showToast(`Phòng này đang được ${other.name} giữ chỗ tới ${other.holdUntil}.`);return}
  const r=getRoom(roomId);
  if(r&&r.status==='occupied'){showToast('Phòng đang có người thuê, không giữ chỗ được.');return}
  // nhả phòng cũ nếu đổi phòng giữ chỗ
  if(t.holdRoomId&&t.holdRoomId!==roomId){
    const old=getRoom(t.holdRoomId);
    if(old&&old.status==='reserved'&&!holderOfRoom(old.id))old.status='available';
  }
  t.holdRoomId=roomId;t.holdFrom=from;t.holdUntil=until;t.holdAmount=amount;
  t.holdNote=document.getElementById('holdNote').value.trim();
  t.active=true;
  if(r&&r.status==='available')r.status='reserved';     // phòng → Đã giữ chỗ
  auditLocal('update','tenants',t.id,null,{hold:`${from}→${until}`,amount});
  notifyTenant(t.id,'general','Xác nhận giữ chỗ phòng '+(r?.name||''),
    `Đã nhận cọc giữ chỗ ${money(amount)} cho phòng ${r?.name||''} từ ${from} đến ${until}.`,'');
  saveData();closeModal('holdModal');renderAdmin();renderPublic();
  showToast(`Đã giữ chỗ phòng ${r?.name||''} cho ${t.name} tới ${until}.`);
});
/** Hủy giữ chỗ: trả phòng về Đang trống nếu không còn ai giữ. */
window.cancelHold=function(tenantId){
  if(!canManageHold()){denyHold();return}
  const t=getTenant(tenantId);if(!t||!t.holdRoomId)return;
  if(!confirm(`Hủy giữ chỗ của ${t.name}? Phòng sẽ về trạng thái Đang trống.`))return;
  const r=getRoom(t.holdRoomId);
  t.holdRoomId='';t.holdFrom='';t.holdUntil='';t.holdAmount=0;t.holdNote='';
  if(r&&r.status==='reserved'&&!holderOfRoom(r.id))r.status='available';
  auditLocal('update','tenants',t.id,{hold:'có'},{hold:'đã hủy'});
  saveData();renderAdmin();renderPublic();
  showToast('Đã hủy giữ chỗ — phòng trở lại Đang trống.');
}
...
function leaseCardHtml(l){
  ...
  if(l.status==='draft')acts.push(`<button class="icon-btn" data-evt="click" data-call="openCheckin" data-a1="${l.id}">Nhận phòng</button>`,`<button class="icon-btn" data-evt="click" data-call="openHoldForm", "openLeaseForm" data-a1="${l.id}">Sửa</button>`,`<button class="icon-btn danger" data-evt="click" data-call="cancelHold", "cancelLease" data-a1="${l.id}">Hủy</button>`);
...
function suggestOtherSlot(payload){
  ...
  const free=[...sel.options].map(o=>o.value).filter(v=>v&&!taken.has(v));
  if(free.length){
    sel.value=free[0];
